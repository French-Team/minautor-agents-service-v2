...
> **E5b (croisement fiche/carte -- LEVER DE LA LECON DU RE-AUDIT)** : pour CHAQUE fin citee dans la fiche (retour a X, activer Y, reactiver Z), verifier que l identifiant cX correspond a une fin de `fins.json` (arbre v2) et que le titre de la fin (ex : "FIN - Retour a ORACLE") correspond au sens declare. Une mention textuelle sans identifiant reel est INSUFFISANTE.
> **E5c (conformite du sens -- modele aero R1/R3)** : la fin declaree correspond a la fin reelle : TOUTE fin d agent va vers ORACLE (reactiver-fin <agent> --cible oracle) - JAMAIS reactiver Cerberus directement, JAMAIS activer un autre agent ; c est le pilote qui decide du suivant et ramene a Cerberus en fin de round.
