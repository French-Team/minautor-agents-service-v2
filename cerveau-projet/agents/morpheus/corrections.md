...
| 2026-08-07 | Tests formels de activer-agent-principal v0.3.2 (8/8 + regression 12/12) | Regle de derivation du nommage : id = profil-session- + partie apres le prefixe session- (session-llm-1 -> profil-session-llm-1). BUG DECOUVERT : session[7:] retirait un caractere de trop ("session-" fait 8 caracteres) -> profil-session--llm-1 (double tiret) ; corriger avec session[len("session-"):]. Ajout d un test negatif (aucune ligne profil-session-session-*) - toujours verifier le NEGATIF pour valider une regle IMMUABLE | | Profil session classeur : mettre_a_jour_profil_session (py+sh) ecrit/met a jour profil-session-<session> a chaque action. PIEGE DECOUVERT : le chemin des protections dans les scripts de test etait faux (tests/../../../../tester/protections = 4 niveaux au lieu de 3) -- les protections n etaient JAMAIS chargees dans test-001 v0.3.0 (source silencieux). Corrige dans test-001 ET test-002 (../../../tester/protections). Lecon : TOUJOURS verifier que les sources des protections aboutissent (echo du PROTECTIONS_DIR + ls) avant de declarer un test valide |
