...
## [LECON] 2026-08-22 -- GENERALISATION INTER-ROUND VULCAIN + THEMIS (Buffy)

**Contexte** : test reel Morpheus+Janus VALIDE (3/3). Generalisation aux autres agents.

**Vulcain v0.6.4** : c14 NON -> c14ir (INTER-ROUND). Avant: boucle infinie (c14 -> c14). Apres: active l agent habilite (Buffy pour la carte) avec rapport du blocage, reprise en c15e.

**Themis v0.5.7** : c8d NON -> c8ir (INTER-ROUND). Avant: retour c3 (re-evaluation sans fin). Apres: active l agent habilite avec rapport de l echec, reprise en c12e.

**Buffy** : pas de cas structurel de KO dans sa carte. Ses defauts sont geres par les agents qui l auditent (Themis c12g, Janus c8ir, Morpheus c7ir). Buffy est le RECEPTEUR d inter-round, pas l initiateur.

**Cartes modifiees** : vulcain 0.6.3->0.6.4, themis 0.5.6->0.5.7. Pattern 14 sync fiches OK. valider-cartes-decision CONFORME.

## [LECON] 2026-08-22 -- INTER-ROUND DANS LES CARTES + RETOUR MORPHEUS A VULCAIN (Buffy)

**Diagnostic** : 2 problemes dans les cartes des agents.

**Probleme 1 -- Morpheus** : la carte avait  branche VULCAIN -> Janus. Or la REGLE DELEGATION de sa fiche dit "reactiver VULCAIN quand il attend mon rapport". Corrige : nouvelle case  (FIN - Retour delegation Vulcain) et  (INTER-ROUND pour les KO de tests).

**Probleme 2 -- Janus** : quand Janus constatait des KO en non-regression (c8 NON), il allait vers c9 (lecons) au lieu d'activer l'agent habilite en inter-round. Corrige : branche NON -> nouvelle case  (INTER-ROUND : activer l agent habilite avec rapport).

**Constat** : le protocole-fin-mission v0.2.0 (R2 : erreur hors-perimetre -> activer agent habilite -> reprise de round) existait mais n'avait JAMAIS ete materialise dans les cartes (0 occurrence dans 20+ parcours JSON). La mission M3 de Socrate le confirmat (jamais executee).

**Cartes modifiees** : morpheus v0.5.6->0.5.7, janus v0.5.7 (deja bumpee avant). Pattern 14 sync fiches OK. valider-cartes-decision CONFORME, ASCII 0.
...
## [LECON] 2026-08-20 -- REVISION FICHE CLIO (nouvelles regles README)

**Mission** : reviser la fiche Clio pour ajouter les nouvelles regles (ton 1ere personne, dry-run obligatoire, badges dynamiques).

**Modifications** :
1. Fiche Clio v0.2.1 -> v0.2.2 : ajout de 3 nouvelles regles absolues + 3 nouvelles specialites
2. Corrections Clio : lecon documentee
...
## [LECON] 2026-08-18 -- RE-EDUCATION CARTE JANUS v0.5.0 (Buffy)

**Contexte** : signalement Themis (audit A REVOIR) + Chiron (education
A REVOIR) : la carte Janus v0.4.20 etait structurellement saine mais
pedagogiquement en retard - c1 sans indice GARDE-FOU C1, aucune redirection
outil bloque, c28 sans indice AGENTS HABILITES. Janus n avait JAMAIS ete
re-eduque.

**Corrections appliquees** (editer-parcours via str_replace) :
1. c1 : indice GARDE-FOU C1 (classification, modele cerberus/themis) - demande
   hors branches -> autre -> c27.
...
4. Bump carte 0.4.20 -> 0.5.0 (--mineure --wet via bumper) + sync fiche
   (PARCOURS v0.5.0) + resync cartes-lock automatique (v0.1.5 du bumper).

**Validation** : valider-cartes-decision janus CONFORME (10/10), bumper --tous
0/0, test-021 9/9, test-037 6/6, ASCII 0, LF 0, lock MATCH.

**Lecons** :
1. La re-education d une carte = AJOUT d indices (jamais de changement de
   structure) : le risque de regression est minimal, mais le bump de version
   (--mineure) reste obligatoire + sync fiche (Pattern 14) + resync lock.
2. Les compagnons du bumper (rapports d audit, BDD lecons) qui referencent
   l ANCIENNE version sont des MENTIONS HISTORIQUES (version auditee) : ne
   PAS les adapter (comme corrections.md), le verdict du bumper est KO par
   design mais les mentions d epoque doivent rester.
3. Le modele de conformite pedagogique (re-education Themis v0.4.10) est
   maintenant applique a Janus v0.5.0 : toute carte d agent principal doit
   avoir (a) GARDE-FOU C1 en c1, (b) redirection outil bloque, (c) AGENTS
   HABILITES.

**Verdict** : VALIDE - carte re-eduquee, tests verts.
## [LECON] 2026-08-18 -- RE-EDUCATION CARTES VULCAIN/MORPHEUS/BUFFY v0.5.0 (Buffy)

**Contexte** : signalement Themis (audit A REVOIR) + Chiron (education
A REVOIR) + Janus (controle) : les cartes de Vulcain (0.4.28), Morpheus
(0.4.15) et Buffy (0.4.14) etaient structurellement saines mais
pedagogiquement en retard - c1 sans GARDE-FOU C1, pas de redirection outil
bloque, pas d AGENTS HABILITES.

**Corrections appliquees** (3 cartes, modele Themis v0.4.10 / Janus v0.5.0) :
1. c1 : indice GARDE-FOU C1 (classification, avec les branches exactes de
   chaque carte et la case cible "autre").
...
4. Bump --mineure --wet (0.4.x -> 0.5.0 pour les 3) + sync fiche + resync
   lock (v0.1.5 du bumper).

**Lecons** :
1. LIMITE DE LONGUEUR DES INDICES : les textes regle doivent rester < 160
   caracteres (test-016 point 9, valider-case). Mes premiers textes (208, 173
   caracteres) ont casse test-016 - raccourcir (modele : 115-140 caracteres).
2. La resync lock apres re-ecriture : j ai modifie les textes APRES le bump
   (raccourcissement) -> les 3 locks sont devenus DIFF -> resynchroniser avec
   resynchroniser_cartes_lock du bumper AVANT la non-regression.
3. KO attendus : test-016 point 1 (pin version 0.4.14 -> 0.5.0, adaptation
   Morpheus) et test-057 points 12/13 (verrou SEUL BUFFY, artefact de session
   car je suis Buffy - reverdit sous Janus). Verifier les KO sous la session
   appropriee avant de conclure.
4. Le modele pedagogique est desormais applique aux 6 cartes principales :
   cerberus, themis, janus (deja conformes) + vulcain, morpheus, buffy
   (re-eduquees v0.5.0).

**Verdict** : VALIDE - 3 cartes re-eduquees, tests verts (hors pins et
artefacts de session documentes).
## [LECON] 2026-08-18 -- RE-EDUCATION 10 CARTES SECONDAIRES v0.5.0 (Buffy)

**Contexte** : signalement Themis (audit A REVOIR) + Chiron (education
A REVOIR) + Janus (controle) : les 10 cartes secondaires (atlas, argus,
hygie, clio, hermes, gardien, chiron, athena, promethee, minerve) etaient
structurellement saines mais pedagogiquement en retard - c1 sans GARDE-FOU C1,
pas de redirection outil bloque, pas d AGENTS HABILITES. Aucun n avait ete
eduque.

**Corrections appliquees** (10 cartes, modele Janus v0.5.0) :
1. c1 : indice GARDE-FOU C1 (branches exactes de chaque carte + case cible
   "autre" : c26/c29/c13/c18).
2. Case "hors parcours" : indices REDIRECTION OUTIL BLOQUE + DOMAINES AUTRES
   AGENTS.
3. Case "Activer l agent habilite" : indice AGENTS HABILITES.
4. Chiron = CAS PARTICULIER : c1 est une ACTION (mission unique) -> pas de
   GARDE-FOU C1 classique ; ajout AGENTS HABILITES dans c10 (redirection).
5. Bump --mineure --wet pour les 10 (atlas 0.4.9->0.5.0, argus 0.1.12->0.2.0,
   hygie 0.1.8->0.2.0, clio 0.5.13->0.6.0, hermes 0.1.5->0.2.0, gardien
   0.1.3->0.2.0, chiron 0.1.2->0.2.0, athena 0.3.6->0.4.0, promethee
   0.3.7->0.4.0, minerve 0.3.7->0.4.0) + sync fiche + resync lock auto.

**Lecons** :
1. Le modele pedagogique s applique AUSSI aux cartes secondaires, avec
   ADAPTATION : les agents a mission unique (chiron, c1 action) n ont pas
   besoin du GARDE-FOU C1 classique mais doivent avoir la redirection et la
   liste AGENTS HABILITES. Le test pedagogique : "que fait la carte si le
   verrou bloque un outil ? si la mission est hors perimetre ?"
2. Textes regle < 160 caracteres ET budget pondere des indices <= 3,0 par
   case (test-016) : verifier les deux AVANT d ecrire (mes textes cibles
   etaient 158-155-150-128, budgets finaux 1.0-2.0-2.5 <= 3.0).
3. KO attendu : test-005 point 17 (pin version atlas 0.4.9 -> 0.5.0) ->
   adaptation Morpheus. test-057 artefact de session buffy. Verifier les KO
   sous la session appropriee.

**Verdict** : VALIDE - 10 cartes re-eduquees, tests verts (hors pins).

## [LECON] 2026-08-18 -- PARCOURS AUTO-CORRECTION CHIRON (Buffy)

**Mission** : construire le parcours d'auto-correction dans la carte de Chiron (pilote unique, decision utilisateur) : se re-eduquer, corriger SA carte, activer Themis pour verification, reprendre.

**Implementation** (parcours-chiron.json v0.2.0 -> 0.3.0) :
- c11 : suivant c12 -> c11b (question d'aiguillage)
- c11b : question "les incoherences concernent-elles MA carte ?" (OUI -> c15, NON -> c12)
- c15 : se re-eduquer (relire MA carte, documenter lecon corrections.md + BDD)
- c16 : corriger MA carte via editer-parcours (indice OUTIL editer-parcours : autorise par l'exception pilote test-058 v0.2.3)
- c17 : activer Themis (verification croisee, elle me REACTIVE via c25b)
- c18 : question reprise (OUI -> c12, NON -> c18 attente / c15 si A REVOIR)
- Verifie : valider-cartes CONFORME, fiche sync v0.3.0, lock MATCH, budgets <= 3.0, textes < 160, navigation guider OK sur les 2 branches.

**Lecons** :
1. UNE EXCEPTION D OUTIL DANS UNE CARTE DOIT COUVRIR LES 2 NIVEAUX DU GARDE-FOU : l'exception chiron de test-058 v0.2.3 couvre les indices OUTIL mais PAS la boucle texte ("if o in texte and o in noms"). L'indice c16 editer-parcours est dans le texte ET dans les noms -> KO point 2. L'adaptation du test doit etre coherente sur TOUTES les boucles.
2. LE PILOTE AUTO-CORRECTION EST UN CYCLE COMPLET DANS LA CARTE : detecter (c11b) -> se re-eduquer (c15) -> corriger SA carte (c16) -> verifier par Themis (c17) -> reprendre (c18). La reprise doit etre une QUESTION d'attente (pattern Buffy c8b), pas un suivant lineaire.
3. LE VERROU EST LE VRAI GARDE-FOU, LE TEST N EST QUE LE REFLET : le verrou bloque chiron sur toute cible != parcours-chiron.json (teste manuellement). L'indice editer-parcours dans SA carte est legitime ; le test doit le refleter sans faux positif.
4. LE BUMP PARCOURS SE FAIT AVEC --parcours <nom> (pas un chemin). Le bumper sync la fiche + le lock automatiquement.

## [LECON] 2026-08-18 -- APPLICATION EDUCATION THEMIS COMBOS ASCII (Buffy)

**Mission** : appliquer les corrections de carte de Themis signalees par Chiron (education aux combos ASCII) : ajouter l'indice OUTIL combos-corriger-non-ascii dans c9 + regle ASCII, mentionner les 2 combos dans la fiche.

**Implementation** (parcours-themis.json 0.4.10 -> 0.5.0, fiche sync, lock MATCH) :
- c9 : regle ASCII en tete (pattern 2) + indice OUTIL combos-corriger-non-ascii (--full --dry-run).
- themis.md : table des outils + combo-corriger-ascii + combos-corriger-non-ascii.
- Verifie : valider-cartes CONFORME, verifier-conformite-fiche CONFORME, test-006 19/19, test-058 6/6, bumper 0/0, evaluateur 15 liens preexistants.

**Lecons** :
1. EDUQUER UN AGENT = ASSIGNER L OUTIL DANS SA CARTE : Chiron a diagnostique que Themis avait la regle mais AUCUN outil ASCII assigne -> inoperante. La correction efficace : indice OUTIL dans la case d'ecriture (c9), pas seulement une regle generale.
2. LA CASE D ECRITURE EST LE POINT D INSERTION NATUREL : c9 (ecrire le rapport) est la ou Themis produit de l ASCII -> c est la case a enrichir. Regle ASCII en tete (pattern 2) + outil juste apres.
3. LE BUMP PARCOURS THEMIS PASSE EN MAJEUR (0.4.10 -> 0.5.0) : le bumper decide, la fiche et le lock suivent.
4. LES 12 COMPAGNONS DU BUMP SONT DES MENTIONS HISTORIQUES : rapports dates, corrections -> on ne les adapte pas. Seuls le lock et la fiche sont resynchronises automatiquement.

## [LECON] 2026-08-18 -- MISE A JOUR FICHE CHIRON CAPACITE PILOTE (Buffy)

**Mission** : mettre a jour la fiche chiron.md avec la capacite pilote d auto-correction (carte v0.3.0) apres la mission du parcours d auto-correction.

**Verdict** : fiche chiron.md mise a jour : Parcours v0.1.0 (15 cases) -> v0.3.0 (23 cases), liste des cases completee (c0c/c0e/c11b/c15-c18), EXCEPTION PILOTE ajoutee aux regles absolues 1 et 2, cycle pilote CHIRON -> THEMIS -> CHIRON documente, Limites et Faiblesses nuancees. verifier-conformite-fiche CONFORME, lock MATCH, bumper 0/0, test-058 6/6, evaluateur 0 lien chiron.

**Lecons** :
1. LE BUMPER SYNCHRONISE LA LIGNE PARCOURS, PAS LE RESTE DE LA FICHE : la ligne "REGLE ABSOLUE -- PARCOURS (vX)" est mise a jour par le bumper --parcours, mais la Vue d ensemble (vX, nb cases), la liste des cases, les regles absolues et les limites restent a mettre a jour manuellement par Buffy. La fiche complete = bumper + edition manuelle.
2. LA FICHE DOIT PORTER LA MEME EXCEPTION QUE LA CARTE : quand un parcours recoit une exception (pilote), la fiche doit la documenter PARTOUT ou la regle inverse apparait (regles absolues, workflow, faiblesses, limites) sinon l agent relit une fiche qui contredit sa carte.
3. LE COMPTAGE DES CASES SE FAIT SUR LE JSON, PAS SUR LA FICHE : la fiche disait 15 cases mais le parcours en avait deja 18 (c0c/c0e absents de la liste). Toujours compter les cases du JSON (dict cases) et reporter le total exact dans la Vue d ensemble.

## [LECON] 2026-08-18 -- FICHE CHIRON : DOCUMENTER BRANCHE A REVOIR c18 (Buffy)

**Mission** : documenter la nouvelle branche A REVOIR de c18 dans la fiche chiron.md (le cycle pilote a evolue lors de la verification reelle : c18 a maintenant 3 branches).

**Verdict** : fiche mise a jour : Branches de decision ligne c18 = OUI (CONFORME) -> c12, A REVOIR -> c15 (retour corriger), NON (pas revenue) -> c18 (attendre) ; tableau du cycle pilote c18 complete avec les 3 branches. verifier-conformite-fiche CONFORME, valider-cartes CONFORME (point 10), lock MATCH, bumper 0/0, evaluateur 0 lien chiron, ASCII 0.

**Lecons** :
1. LA FICHE SUIT LE PARCOURS, PAS L INVERSE : quand une case de parcours gagne une branche (c18 : A REVOIR -> c15 lors de la verification reelle), la fiche doit refleter TOUTES les branches dans TOUTES les sections (branches de decision ET tableau du cycle pilote). Une seule mention a jour ne suffit pas.
2. UNE EVOLUTION DE CARTE ISSUE D UN CYCLE PILOTE DOIT REMONTER DANS LA FICHE : la correction c18 a ete faite par Chiron (verrou pilote) + audit Themis CONFORME, mais la fiche (domaine Buffy) etait restee en retard. La chaine : carte corrigee -> fiche mise a jour par Buffy -> verification par Themis.
3. LA VERIFICATION DE FICHE APRES MODIFICATION = verifier-conformite-fiche + valider-cartes point 10 (coherence fiche/parcours) + lock MATCH : les 3 gardent la fiche et le parcours coherents.

## [LECON] 2026-08-18 -- TABLEAU AGENTS DISPONIBLES COMPLETE (Buffy)

**Mission** : completer le tableau "Agents disponibles" de cerberus.md (lignes 202-213) qui ne listait que 10 agents (Buffy->Themis) : 5 agents secondaires manquants (Argus, Chiron, Gardien, Hermes, Hygie).

**Verdict** : tableau complete (15 agents) avec les roles exacts de AGENTS.md + "Quand l'activer" depuis les fiches. valider-tableaux : plus que le faux positif classeur-variables (dossier de donnees type classeur, pas un agent). verifier-conformite-fiche CONFORME, bumper 0/0, evaluateur 15 preexistants (0 nouveau), ASCII 0.

**Lecons** :
1. LE TABLEAU AGENTS DISPONIBLES DE CERBERUS EST LA CARTE D ENTREE DU ROUTEUR : il doit lister TOUS les agents (valider-tableaux verifie la completude vs dossiers agents/*/ contenant une fiche X.md). Un agent oublie = jamais active par Cerberus.
2. L OUTIL VALIDER-TABLEAUX DETECTE LES CLASSES PAR LEUR FICHIER <dossier>.md : classeur-variables est un dossier de donnees (type: classeur) avec classeur-variables.md -> faux positif de completude (il n a pas de fiche agent). A ne pas ajouter au tableau.
3. LES ROLES DU TABLEAU VIENNENT DE AGENTS.md + FICHES : le "Quand l'activer" doit etre operationnel (la condition d activation de la carte de l agent), pas un copier-coller du role.

- **2026-08-19 (test-055 : indice editer-parcours)** : toute mention d un outil dans une REGLE doit avoir un indice outil (nom identique, type outil) dans la MEME case. Les cases de delegation (argus c29a, athena c19, atlas c27, clio c14, gardien c29a, hermes c29a, hygie c29a, janus c28, minerve c19, promethee c19) mentionnaient editer-parcours dans la regle 'AGENTS HABILITES' sans indice -> 10 ecarts test-055. Correctif : indice outil editer-parcours ajoute (catalogue/chemin/nom/type). test-055 12/12, test-006 19/19, 10 cartes CONFORMES.

---

## 2026-08-19 - Correction des 10 liens relatifs faux (mission liens casses)

**Action** : corrige 10 liens relatifs faux dans 4 fichiers du cerveau-projet :
9 dans les ebauches de protocoles (creation-scripts-temporaires, nettoyage,
verification-coherence) + 1 dans pense-betes/specs/index-spec.md. Les cibles
existaient toutes : seuls les chemins relatifs etaient faux (../ manquants,
test-046 renomme en test-046-hermes-fautes, spec refonte deplacee dans
docs-dev-cerveau-projet/). Resultat : l evaluateur de coherence passe de 15 a
5 liens (les 5 restants sont des exemples de format [protocole-X/] dans les
lecons - faux positifs a ignorer dans l outil, volet Vulcain).

**Lecon** : les fichiers .ebauche.md (brouillons de protocoles) contiennent des
liens relatifs qui cassent quand on deplace/renomme leurs cibles. La regle :
quand on deplace un fichier cible (spec, test, regle), verifier les liens des
...
## [LECON] 2026-08-20 -- REPARATION PARCOURS BUFFY : controles apres actions (Buffy)

**Contexte** : la branche 'agent' du parcours Buffy sautait les etapes de creation (c18-c21) car les cases etaient de type action sans verification.

**Correction** : 4 controles ajoutes :
- c18b : 'Nom valide ?' apres c18 (verification nom)
- c19b : 'Dossier cree ?' apres c19 (creation dossier)
- c20b : 'Fichiers crees ?' apres c20 (copie template)
- c21b : 'AGENTS.md a jour ?' apres c21 (MAJ AGENTS.md)

Chaque controle reboucle sur l'action si NON.

**Lecon** : les cases d'action sans controle sont des BOMBE A RETARDENMENT. Le LLM saute les etapes car rien ne le force a faire le travail. TODOULTE case d'action doit etre suivie d'un controle qui verifie que le travail a ete fait.

**Regle ajoutee** : AJOUTER UN CONTROLE APRES CHAQUE CASE D'ACTION dans les parcours. Pas d'exception.
[LECON 2026-08-20] Reparation immediate erreurs hors mission (regle utilisateur) : 3 problemes au scan global evaluer-processus reparables dans le round - 1) vulcain tester-lancer-non-regression en mode direct : corriger le mode en verrou-dev (liste blanche DEV_NON_REGRESSION, ignore par evaluer-processus) ; 2) janus proteger-verrou-marbre : DECLARATION_FAUTIVE (outil exclusif gardien), retirer l entree du registre ; 3) janus ajouter-contenu-fichier : OUTIL_HORS_CARTE, ajouter l indice a la carte janus case c9 lecons (bump 0.5.2 -> 0.5.3 + sync fiche + lock). Resultat : evaluer-processus global 0 probleme, janus 0, cerberus 0, valider-cartes-decision CONFORME, marbre 8/8.
LECON 2026-08-20] Reparation carte themis (indice evaluer-processus c16) : ajout via editer-parcours --modifier-case (dry-run puis wet), bump 0.5.2 -> 0.5.3, description mise a jour, lock resync, fiche themis synchronisee (Pattern 14). Valide : valider-cartes-decision CONFORME, valider-case CONFORME (0/0/0), evaluer-processus global + themis 0 probleme, marbre 8/8, ASCII 0. LECON TECHNIQUE : sous Windows, pathlib.write_text convertit les LF en CRLF -> apres toute modification directe d un JSON, reecrire en write_bytes (json.dumps + \n encode) pour preserver les LF purs (comme editer-parcours). Verifier CRLF apres chaque ecriture JSON.
LECON 2026-08-20] Reparation 2 ecarts socrate (signales par Themis) : 1) champ parcours de parcours-socrate.json complete (nom: parcours-socrate, agent: socrate, description) - reecriture write_bytes LF purs, guider --liste affiche maintenant Agent: socrate ; 2) mention REGLE ABSOLUE -- PARCOURS (v0.1.0) ajoutee dans socrate.md (Pattern 14) - valider-cartes-decision socrate CONFORME dont P10 (Fiche PARCOURS (v0.1.0) == parcours 0.1.0), valider-case CONFORME, ASCII 0 CRLF 0, marbre 8/8. ERREURS HORS MISSION REPAREES (regle utilisateur) : 2 DECLARATION_FAUTIVE valider-case (janus 22:55 + themis 23:02, outil EXCLUSIF buffy, absents des cartes et combos) retirees du registre - evaluer-processus global 0 probleme. Lecon : valider-case est exclusif a buffy - janus/themis ne doivent pas l utiliser manuellement (verification par valider-cartes-decision si habilite ou inspection JSON).
[LECON 2026-08-20] Reparation immediate CRLF missions-revision.md (point Themis, autorisation utilisateur) : fichier de travail de Socrate cree a 20:16 portait 28 CRLF (fins de ligne Windows). Correction : lecture binaire + replace CRLF -> LF + reecriture write_bytes (lecon #180) - ASCII 0 CRLF 0, contenu inchange (1571 -> 1543 octets, seules les fins de ligne changent). Scan complet dossier socrate : 0 fichier restant avec CRLF ou non-ASCII. Combo corriger-fichier NON adapte a ce type de fichier (corriger-nommage exige --type protocole/agent/outil/convention, or missions-revision.md est un fichier de travail) - pour les CRLF, la reecriture binaire directe est la bonne methode. Lecon : les fichiers de travail crees par les agents (missions, rapports) doivent aussi respecter LF pur - verifier CRLF sur TOUT fichier modifie ou cree, pas seulement les JSON.

[LECON 2026-08-20] MISSION 2 VOLETS (demande utilisateur). VOLET 2 - AGENTS-historique.md : les interventions du 21/08 (7 entrees Cerberus/buffy/vulcain/themis) avaient ete inserees dans la section 20/08 par le bug de bornage de activer-agent-principal (corrige par Vulcain v0.5.21). Correction : deplacement des 6 entrees de la section 20/08 vers la 21/08 + regeneration de l encart Activites recentes (fonction du tool reutilisee). Structure finale : 21/08 = buffy 2, Cerberus 5, themis 3, vulcain 1 ; encart = 10 entrees toutes classees 21/08. Lecon : apres un fix d insertion, les entrees deja ecrites restent dans les mauvaises sections - verifier la repartition par section ET regenerer l encart (qui se deduit du corps).
VOLET 1 - CASE COMPRENDRE L OUTIL (demande utilisateur) : quand un agent a un probleme avec un outil (erreur de commande, fonctionnement oublie), il ne doit PAS lire le code pour comprendre : le mode d emploi est dans le .md. Ajout dans les 20 cartes (17 agents + 3 revision) d une deviation cU1 (question Probleme avec un outil ? OUI->cU2 NON->suite) -> cU2 (action COMPRENDRE L OUTIL : lire le .md + verifier combo avec consulter-combos + generateurs-commande) -> cU3 (fin Reprendre ma mission : relancer --case <cid> d origine). Cablage : c0d.suivant->cU1 (9 cartes), c0c.suivant->cU1 (10 cartes), socrate c0cb.OUI->cU1. Versions bumpees +0.1 (Pattern 14 fiche PARCOURS synchronisee), lock 20 cartes resynchronise (les 3 revision n y etaient pas - ajoutees). Validations : valider-cartes-decision 17/17 CONFORME, valider-case 0 erreur cU, marbre exit 0, ASCII 0 CRLF 0, guider cU1->cU2->cU3 OK. LECON : une deviation jointe au flux principal (BFS) rend ses fins joignables pour valider-case - une fin detachee est NON JOIGNABLE. LECON : sous Windows glob() renvoie des chemins avec backslashes et os.path.join(RACINE='.') prefixe ./ - pour matcher les cles du lock, normaliser avec replace('\','/') et retirer le prefixe ./.

[LECON 2026-08-21] REPARATION OPTION A (retour Pattern 8, demande utilisateur - rapport Themis E1-E5) : la boucle de chaine doit continuer sans repasser par Cerberus - l agent suivant active l agent suivant. E1 ROUTAGE (buffy) : c16 question delegation + branche 'tests' -> c31b ; c31 (Activer Vulcain) transformee ACTION->FIN avec chaine complete (Vulcain -> Morpheus tests -> Janus controle -> Cerberus) ; c31b NOUVELLE fin Activer Morpheus (tests). E4 RELAIS : message standard 'MESSAGE DE RELAIS A L AGENT SUIVANT : COMMENCE ton travail, FINIS-le (RVAV + lecons), puis ACTIVE l agent suivant selon TA carte. Ne REACTIVE Cerberus QUE si tu es le DERNIER maillon' ajoute dans 22 fins Activer X (16 cartes). E2 HARMONISATION : les 3 chaines contradictoires (buffy c31 'Vulcain->Janus->Clio->Cerberus' / vulcain c9/c15 'Vulcain->Morpheus->Janus->Cerberus' / buffy c35 'Vulcain reactive l agent precedent') convergent sur le modele de reference VULCAIN -> MORPHEUS (tests) -> JANUS (controle) -> CERBERUS ; corrige buffy c31/c35/c35d + vulcain c16d (Pattern 13 : dernier maillon -> reactiver, maillon -> activer suivant). E5 REFERENCES : 'ma fin normale c13 (FIN - Reactiver Cerberus)' etait COPIE dans les fins de reprise de TOUTES les cartes (buffy c15e, clio c10e, vulcain c9e/c15e, athena, atlas, cerberus, janus, minerve, morpheus, promethee, themis) - remplacee par la fin normale REELLE de chaque carte (buffy c8, clio c12, vulcain c9/c15, athena c10, atlas c11, cerberus c20, janus c10, minerve c10, morpheus c10, promethee c10, themis c13). Versions bumpees +0.1 (16 cartes), fiches PARCOURS synchronisees, lock 20 cartes resynchronise. Validations : valider-cartes-decision 17/17 CONFORME, valider-case buffy CONFORME (les NON CONFORME cerberus c45/chiron c18 sont preexistants), marbre exit 0, ASCII 0 CRLF 0, guider c16->outil->c31 et c16->tests->c31b fonctionnels. SUITE : Vulcain doit adapter le garde-fou v0.5.19 de activer-agent-principal pour AUTORISER l activation de chaine legitime (activer <session> <agent> en fin de carte) au lieu de tout bloquer sauf --forcer (E3 du rapport Themis) - sans cela les fins Activer X restent bloquees. LECON : un message de fin copie d un parcours a l autre reference des cases qui n existent pas dans la carte cible - verifier les references de TOUTES les cartes (grep 'c13 (FIN - Reactiver Cerberus)'), pas seulement celles signalees.
[LECON 2026-08-21] BOUCLE KO JANUS - REPARATION CARTES CERBERUS + INTEGRATION SOCRATE : 1) CARTE CERBERUS : cases c45/c45b/c46/c46b ajoutees avec format NON CONFORME (branche_vraie/branche_fausse au lieu de branches[] ; c45/c46 sans champ suivant) - corrigees via editer-parcours --modifier-case (dry-run puis wet), bump 0.5.8 -> 0.5.9, fiche synchronisee (Pattern 14). 2) SOCRATE : 4 parcours (socrate + 3 revision-*) - fins 'Reactiver Cerberus' remplacees par 'FIN - Activer Janus' (REGLE IMMUABLE JANUS : seule la carte janus a la fin Reactiver Cerberus) ; case orpheline c1b recablee (c1.suivant c2 -> c1b, bump 0.1.1 -> 0.1.2) ; socrate ajoute au tableau des agents de cerberus.md ; cerberus.md CONFORME valider-tableaux. 3) OUTIL convertir-carte-mermaid : BUG multi-parcours - les 4 parcours socrate partageaient appartient_a -> meme fichier de sortie (socrate.mmd ecrase). Correctif : nom_fichier_parcours() derive le nom du FICHIER SOURCE (parcours-<agent>.json -> <agent>, sous-parcours -> <agent>-<sous>), filtre --agent conserve appartient_a. Bump 0.2.0 -> 0.2.1 (py + md). 4) PINS DE TESTS adaptes (reparations immediates hors mission) : test-013 (version cerberus 0.5.9), test-072 (agent resolu via identite.appartient_a au lieu du nom de fichier - les revision-* relisent la fiche socrate). Validations : valider-cartes-decision socrate+cerberus CONFORME, valider-case 0 erreur, marbre 8/8, test-096 11/11 + 20 cartes synchronisees, test-070 13/13, test-072 10/10, test-094 7/7, ASCII 0 CRLF 0, lock 4 parcours socrate resynchronise. LECON : un agent multi-parcours (socrate + sous-parcours revision-*) casse les outils qui nomment par agent - toujours nommer les sorties par FICHIER SOURCE (unique), pas par appartient_a (partage). LECON : le KO contextuel verrou (test-026 point 8) pendant un relais est ATTENDU - l outil detecter-cablages-manquants tourne en tant que janus, la session est sur buffy ; il passera quand janus reprendra.
[LECON 2026-08-21] BUG DEMARRAGE AGENTS CASSE (signalement utilisateur : themis activee sans demarrer) : le bloc DEMARRAGE OBLIGATOIRE de activer-agent-principal.py (instruction_demarrage, ligne 242) ecrivait '\\n' en Python -> le LITTERAL backslash+n (0x5C 0x6E) etait grave dans AGENTS.md au lieu d un vrai retour a la ligne. L agent active copiait 'python3 ...guider-parcours.py \n cerveau-projet/...' -> python essayait d ouvrir le FICHIER LITTERAL '\n' -> ERREUR au demarrage (cas themis : activee mais jamais demarree). Correctif : '\\n' -> '\\\n' (backslash de continuation + vrai newline, parite avec le .sh ligne 936 qui ecrit '\' + newline). Bloc deja grave dans AGENTS.md (themis) corrige en place (remplacement binaire octet par octet : 0x5C 0x6E -> 0x5C 0x0A). Verifie : instruction_demarrage() s execute (commande valide), la commande copiee-collee fonctionne reellement. LECON : en Python, '\\n' = litteral backslash+n (2 octets), '\n' = backslash + vrai newline, '\' + '\n' = backslash de continuation + newline - TOUJOURS tester la commande generee avec repr() avant de valider un bloc grave pour les agents. LECON : un agent active qui ne demarre pas = souvent la commande de lancement du parcours est inexecutable - verifier le bloc DEMARRAGE dans AGENTS.md (pas de litteral \n).
[LECON 2026-08-21] REPARATION D1 (boucle KO audit Themis) : cartes-lock.json desynchronise pour parcours-cerberus.json - l empreinte du lock (35c8...) ne correspondait pas au fichier (49b2...) selon la fonction officielle d editer-parcours (empreinte_fichier : LF + rstrip par ligne). CAUSE : la description de la carte a ete modifiee en write_bytes direct APRES le bump via editer-parcours (qui resynchronise le lock a chaque ecriture) - la carte (18:33) etait plus recente que le lock (18:11). IMPACT : la prochaine edition via editer-parcours aurait ete BLOQUEE (anti-contournement barrage n3). CORRECTION : resynchronisation ciblee du lock avec la fonction officielle (json.dump indent=1 + newline LF) - 20/20 cartes synchronisees, ASCII 0 CRLF 0, evaluer-processus buffy 0 probleme, test-057 22/24 (2 KO contextuels verrou : le test tourne sous buffy donc le verrou ne bloque pas - OK sous janus). LECON : MA lecon precedente (2026-08-20) sur write_bytes qui desynchronise le lock etait VRAIE - j ai refait la meme erreur dans cette mission en modifiant la description en write_bytes sans resynchroniser. APRES TOUTE ecriture directe d une carte, recalcule TOUJOURS l empreinte avec la fonction officielle (LF + rstrip par ligne) ET compare au lock - ou mieux : passer par editer-parcours pour toute modification, meme cosmetique.
[LECON 2026-08-21] ALIGNEMENT ALIAS -> CANONIQUE (demande utilisateur) : 34 indices de 16 cartes portaient l'alias corriger-symboles au lieu du nom canonique corriger-accents-zones-sensibles (seule famille multi-noms a violer la convention). Renommage via editer-parcours --modifier-case (34 cases) + bump PATCH via mettre-a-jour-versions --parcours (16 cartes, sync fiche Pattern 14 + resync lock auto) + pins adaptes (test-005 atlas 0.5.4, test-013 cerberus 0.5.10, test-016 buffy 0.5.4) + vues mermaid/SVG regenerees (test-096 11/11). Valide : valider-cartes-decision 17/17 CONFORME, test-055 12/12 (regle texte vulcain c7 corrigee en meme temps), test-035 10/10, marbre 8/8, ASCII/LF 0. LECON : aligner les cartes sur le canonique est la vraie correction quand registre (canonique) et cartes (alias) se contredisent - pas reverter le registre ni patcher l'evaluateur.
[LECON 2026-08-21] MODE DISCUSSION + REDACTION (demande utilisateur) : capture des 4 decisions v2 dans proposition-v2.md - D1 carte de decision -> ARBRE DES DECISIONS (systeme veineux : branche/theme -> categories -> cases d execution -> cases suivantes -> case de fin ; plus de depart unique ni de fin unique ; le bon chemin choisi des le depart = fin optimale) ; D2 outils toujours dynamiques + suite de non-regression freelance SEPAREE de la suite actuelle (objectifs et contrats differents) ; D3 activation et sauts de cases AUTOMATISES : systeme TRANSPARENT pour les agents (les actions recurrentes de la v1 deviennent invisibles) ; D4 standard : abandon ASCII+LF+bannir-emojis -> UTF8 + CRLF + emojis (standard actuel). Structure : section 0 journal des transmissions + P9 modifie + section 4 (arbre) + 6bis (non-regression separee) + 5 (activation automatisee) + 9 (etapes). Le fichier proposition-v2.md reste en ASCII/LF (il vit dans le workspace v1, test-097 ASCII) - D4 s'appliquera aux fichiers construits dans freelance/. Lecon : en mode discussion/redaction, documenter d'abord dans un journal avant toute construction.
[LECON 2026-08-21] BOUCLE KO THEMIS - CORRECTION 2 ECARTS proposition-v2.md : (1) annexe ligne 268 'Regles immuables (ASCII, LF, veracite) | Conserves (P9)' -> 'Regles immuables (veracite) | Conservees ; ASCII/LF REMPLACES par UTF-8 + CRLF + emojis (D4)' ; (2) liste a conserver ligne 43 'marbre, ASCII, LF, veracite' -> 'marbre, veracite - avec abandon du standard ASCII/LF au profit de UTF-8 + CRLF + emojis (D4)'. Verifie : 0 mention incoherente, ASCII/LF 0. LECON : une decision de changement de standard (D4) se propage dans TOUTES les zones du fichier, meme les listes historiques 'a conserver' et l'annexe.
[LECON 2026-08-21] MODE DISCUSSION + REDACTION (suite) : D5 capturee - arbre des decisions = REDIRECTIONS vers fichiers. 1) depart = choix d UN THEME -> suite de redirections (besoin de X -> commande / parcours / combo). 2) allegement : long parcours = fichier separe, la case = simple lien. 3) fins centralisees : fichier unique de toutes les fins, une case = lien + fin. 4) transparence : passages entre fichiers automatises et invisibles (D3). Ajoute dans proposition-v2.md : journal D5 + regles D5 + squelette reecrit + mecanique + benefices. ASCII/LF 0.
[LECON 2026-08-21] MODE DISCUSSION + REDACTION (suite) : D6 capturee - transparence = commande simple + outils formulaire. 1) activation = UNE simple commande qui cache PLUSIEURS outils (ils se lancent automatiquement, plus de lancement manuel un par un v1). 2) outil = FORMULAIRE : l agent lance l outil, remplit les champs, valide ; l OUTIL compose la commande complete et l envoie (l agent ne connait plus la syntaxe). Ajoute dans proposition-v2.md : journal D6 + regle Commande simple (s5) + regle Outil=formulaire + mecanique transparence 4 points (s6). ASCII/LF 0.
[LECON 2026-08-21] MODE DISCUSSION + REDACTION (suite) : D7 capturee - format du formulaire d'outil v2. Formulaire DECLARATIF JSON (outil, version, champs) ; champs types (nom, type texte/nombre/boolean/liste/fichier/enum, requis, defaut, description, valeurs) ; validation (type, requis, plage, enum, coherence, message clair, REFUS AVANT execution) ; contrat (le formulaire EST le contrat, .md documente chaque champ P1, schema derive du formulaire sans duplication) ; exemple concret lire-fichier. Sous-section 'Format du formulaire d outil (D7)' dans section 6 + journal D7. ASCII/LF 0. Coquilles corrigees (boolen->boolean, declaration declarative).
[LECON 2026-08-21] MODE DISCUSSION + REDACTION (suite) : D8 capturee - themes concrets de l'arbre v2. 9 themes (branches) : CREER, MODIFIER, LIRE, VALIDER, TESTER, REDIGER, NETTOYER, COORDONNER, EXPLORER - chacun = une suite de redirections (D5) besoin de X -> commande/parcours/combo. Sous-section 'Themes de l arbre (D8)' dans section 4 : tableau 9 themes + arbre concret + regles (nommage MAJUSCULES, parcours separes pour les longs themes, extensible, PROPOSITION a valider). Coquille corrigee (contrler->controler). ASCII/LF 0.
[LECON 2026-08-21] MODE DISCUSSION + REDACTION (suite) : D9 capturee - historique par agent + tokens-historique.md (PAS de trace unique). 1) l'utilisateur refuse l'historique.jsonl unique -> historique PAR AGENT comme v1 (historique-agents/historique-<agent>.md) ; 2) les outils S'ENREGISTRENT EUX-MEMES (auto-journalisation des usages, l'agent ne declare plus) ; 3) tokens-historique.md : tableau des activites recentes + tokens consommes/envoyes/recus/en cache. Modifie : journal D9, s1 (probleme historique), s3 arborescence (traces/ -> historique/ avec 3 sous-zones), s6 (regle auto-enregistrement + sous-section tokens avec exemple). ASCII/LF 0.
[LECON 2026-08-21] MODE DISCUSSION + REDACTION (suite) : D10 capturee - BDD des lecons v2 revue (bible). 1) lecons CLASSEES ET CATEGORISEES (categories, sous-themes, index, recherche, tags) ; 2) TABLE DES 20 DERNIERES LECONS (date, agent, categorie, titre) ; 3) CASES DE L'ARBRE pour consulter comme une bible au moment du besoin - theme LECONS ajoute aux themes D8 (3 redirections : 20 dernieres, par categorie, parcours lecons-bible fichier separe). Sous-section 'La BDD des lecons v2 (D10)' dans section 4 (4 parties). Corrections ASCII : classee->classee. ASCII/LF 0.
[LECON 2026-08-21] MODE DISCUSSION + REDACTION (suite) : D10 capturee - BDD des lecons v2 revue (bible). 1) lecons CLASSEES ET CATEGORISEES (categories, sous-themes, index, recherche, tags) ; 2) TABLE DES 20 DERNIERES LECONS (date, agent, categorie, titre) ; 3) CASES DE L'ARBRE pour consulter comme une bible au moment du besoin - theme LECONS ajoute aux themes D8 (3 redirections : 20 dernieres, par categorie, parcours lecons-bible fichier separe). Sous-section 'La BDD des lecons v2 (D10)' dans section 4 (4 parties). Corrections ASCII : classee. ASCII/LF 0.

[LECON 2026-08-21] CREATION AGENT REDACTEUR-V2 (demande utilisateur : agent ENTIEREMENT DEDIE a la redaction des docs v2, pro des pro, ROUND SOLO) : cree la fiche redacteur-v2.md (profil redaction docs v2), corrections.md, parcours-redacteur-v2.json (structure SOLO : c0 relire -> c0b -> c0e lecons -> c0c contexte -> cU1 probleme outil -> c1 mission -> c2 rediger -> c3/c4 auto-validation -> c5 RVAV -> c6 lecons/usages -> c7 FIN Activer Cerberus - PAS de maillon Themis/Janus car round solo). Integration systeme : AGENTS.md (liste groupe 2 + tableau agents), cartes-lock.json resynchronise (empreinte officielle LF + rstrip), activer-agent-principal py + sh (couleur #7c3aed + triplet role/fiche/corrections, bump 0.5.23 -> 0.5.24), vues mermaid/SVG + index.md (test-096 11/11, 21 cartes synchronisees). Validations : valider-cartes-decision CONFORME (dont P8 commande activer exacte dans les fins + P10 Pattern 14), valider-case CONFORME 0/0/0, test-072 10/10 (c0 2 outils lire-fichier + champ nom sur CHAQUE indice outil), test-055 12/12, test-013 22/22, test-079 15/15, test-099 6/6, marbre 8/8, ASCII 0 CRLF 0. LECON 1 : les indices outil des cartes exigent le champ 'nom' (egale au catalogue) - sans lui, test-072 et test-055 les comptent a 0 et signalent des ecarts regle/indice. LECON 2 : sous Windows, json.dump en mode texte cree des CRLF + reformate l indentation - reecrire les JSON en write_bytes/json.dump indent=1 + newline='
' (comme la fonction officielle resynchroniser_cartes_lock) pour preserver LF purs et format identique. LECON 3 : un agent SOLO (fin = reactiver Cerberus) doit TOUT verifier lui-meme (validations + tests) car il n y a pas de Themis/Janus dans sa chaine.


## [LECON] 2026-08-22 -- ALIGNEMENT FIN DE MISSION REDACTEUR-V2 (MODE CONVERSATION)

**Contexte** : arbitrage utilisateur - redacteur-v2 reste actif en conversation apres sa redaction, et ne reactive Cerberus QUE sur la phrase FIN DE CYCLE.

**Corrections** :
1. AGENTS.md tableau : ROUND SOLO -> MODE CONVERSATION (reactive Cerberus sur fin de cycle).
2. Carte parcours-redacteur-v2.json : c7 (fin avec reference morte vers: conversation) transformee en QUESTION de re-entree (OUI -> c1 boucle conversation / FIN DE CYCLE -> c8) + c8 (fin reactiver Cerberus) ajoutee.
3. Description du parcours alignee (round SOLO -> MODE CONVERSATION).
4. Pattern 14 : fiche sync (v0.1.5) + lock cartes-lock resynchronise.

**Lecons** :
1. UNE CASE fin AVEC vers TOP-LEVEL EST UNE REFERENCE MORTE : les validateurs ne verifient que suivant et branches[].vers, jamais le vers d une fin - le mode conversation etait annonce mais AUCUNE case de re-entree n existait (flux lineaire c0->c7).
2. LE ROLE AGENT D UN AGENT EST GRAVE EN DUR DANS activer-agent-principal.py/.sh : modifier le bloc session d AGENTS.md a la main est ECRASE a la prochaine activation - aligner le dictionnaire AGENTS de l outil (Vulcain) en plus.
3. ECRITURE DIRECTE D UNE CARTE = DESYNC DU LOCK : editer la description d un parcours via editer-fichier desynchronise cartes-lock.json (anti-contournement) - resynchroniser via mettre-a-jour-versions --parcours --wet (qui bumpe + resync lock).
## [LECON] 2026-08-23 -- CORRECTIONS DE FORMATION DE CLIO POUR README-V2 (Buffy)

**Contexte** : mission de correction de la fiche + carte de Clio pour la redaction de README-v2.md (defauts signales par Chiron rapport-verification-clio-readme-v2-2026-08-23 E1-E5, verdict Janus VALIDE). Session interrompue a 21:13, reprise.

**Actions** :
1. Verification de l etat : la session interrompue avait deja ajoute la branche readme-v2 (c1), les cases c22/c23 et l EXCEPTION REDACTION V2 dans la fiche, avec bump 0.6.4 -> 0.6.5.
2. ANOMALIE TROUVEE : la branche c1 readme-v2 pointait vers c21 qui n existait PAS (les cases c22/c23 etaient creees mais non cablees) -> guider --liste en ERREUR. Correction via editer-parcours --branche c1 --reponse readme-v2 --cible c22 (dry-run puis wet), lock resynchronise.
3. Pattern 14 : fiche clio.md synchronisee (PARCOURS v0.6.4 -> v0.6.5).
4. E3 : sources de verite v2 ajoutees dans la section Connexions de la fiche (freelance/docs/, freelance/protocoles/, freelance/regles/, freelance/conventions/, tools-commun/, jarvis/).
5. Validations : valider-cartes-decision clio CONFORME (10/10 dont P10 Pattern 14), valider-case 0 erreur (1 alleger c20 PRE-EXISTANT hors mission), ASCII 0/0 sur fiche + carte, navigation readme-v2 OK (c1 -> c22 -> c23), combo controle-impacts termine.

**Lecon** : une case creee sans cablage (branche -> cible inexistante) laisse le JSON structurellement valide mais casse la navigation (guider --liste en ERREUR) : apres tout ajout de cases de parcours, verifier la navigation reelle du nouveau chemin (guider --liste + --reponses) et pas seulement valider-cartes-decision. Verdict : VALIDE.
## [LECON] 2026-08-23 -- REPARATION INTER-ROUND D1 : CARTE THEMIS COUVRE valider-conformite-ascii (Buffy)

**Contexte** : inter-round lance par Janus (controle final de la chaine corrections Clio readme-v2) : evaluer-processus detectait OUTIL_HORS_CARTE themis -> valider-conformite-ascii (usage declare au registre par Themis mais absent des indices de la carte themis, qui ordonnait combos-corriger-non-ascii en c9).

**Actions** : ajout de l indice outil valider-conformite-ascii (commande sur <fichier-rapport>) dans la case c9 de la carte themis via editer-parcours --modifier-case, bump version 0.5.8 -> 0.5.9, sync fiche themis PARCOURS (v0.5.9) (Pattern 14), lock resynchronise. Validations : valider-cartes-decision themis CONFORME 10/10, evaluer-processus 0 probleme, ASCII 0/0.

**Lecon** : un controleur (Themis/Janus) qui utilise un outil d audit manuel (valider-conformite-ascii sur un rapport) cree un usage au registre que SA carte doit couvrir - le combo projet entier (combos-corriger-non-ascii --full) et la verification fichier-unique (valider-conformite-ascii) sont 2 outils complementaires a declarer dans la case d ecriture. La boucle KO fonctionne : Janus signale -> Buffy repare la carte -> re-controle. Verdict : VALIDE.
## [LECON] 2026-08-23 -- REPARATION IMMEDIATE (Cerberus) : DELEGATION EN INTER-ROUND (Buffy)

**Contexte** : Cerberus (deviation c12b) m a activee pour la reparation immediate des erreurs hors mission du bilan consolide de la chaine clio-readme-v2. 3 lots : (1) CRLF clio/corrections.md + 9 residus -> Hygie ; (2) 3 divergences d outils -> Vulcain.

**Deroulement** : branche c1 'autre' -> c33 'autre-agent' -> c34/c34b/c36 (FIN - Delegation). Active Hygie (reparation lot 1 : snapshot 8417, 9/9 residus supprimes, CRLF corrige, rapport) puis Vulcain (lot 2 : 3 versions alignees, 0 DIVERGENT, tests Morpheus VALIDE). Chaque habilite m a reactivee en fin d inter-round (protocole v0.2.0 R2/R3).

**Lecons** :
1. UNE MISSION DE REPARATION HORS PERIMETRE = DELEGATION (c36), PAS une modification directe : router vers les agents habilites (Hygie, Vulcain) au lieu de corriger soi-meme.
2. PIEGE COMMANDE REACTIVER : reactiver <session> <raison> <agent> va TOUJOURS vers CERBERUS (nom trompeur) - pour reactiver l APPELANT dans un inter-round, utiliser ACTIVER <session> <agent> <raison> (comme l a montre Hygie qui a d abord fait reactiver -> Cerberus, corrige ensuite).
3. Le protocole v0.2.0 R3 : en inter-round, l habilite reactive l APPELANT (PAS Janus, PAS Cerberus) qui reprend son round ; le dernier habilite (ici moi) reactive Cerberus avec le bilan consolide.
