...
# Version : 0.1.0-beta
# Statut : ebauche
# Parite avec generateurs-case.py (meme comportement, memes resultats).
...
# La racine du projet est calculee ici (pas de __file__ en mode stdin)
# et transmise via la variable d'environnement GC_RACINE.
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
export GC_RACINE="$(cd "$SCRIPT_DIR/../../../../.." && pwd)"
python3 - "$@" << 'PYEOF'
# generateurs-case (code embarque dans le .sh -- parite avec le .py)
import argparse
import json
import subprocess
import sys
...
import os
RACINE = Path(os.environ.get("GC_RACINE", "."))
...
def construire_case(args, donnees):
    ...
    elif type_case in ("question", "indice") and args.suivant:
...
def action_ajouter(args):
    ...
    if args.apres:
        ...
        if not args.suivant:
            if case.get("type") in ("indice", "question", "controle") and ancien_suivant:
...
def action_supprimer(args):
    ...
    if cible is None:
        print(_couleur("ERREUR: la case '%s' n'a pas de suivant et --vers absent : impossible de recabler" % args.case_id, "rouge"), file=sys.stderr)
        sys.exit(1)
    ...
    if args.dry_run:
        print(_couleur("[DRY-RUN] Case '%s' supprimee, %d reference(s) recablee(s) vers '%s'" % (args.case_id, len(refs), cible), "jaune"))
        return 0
    sauvegarder_parcours(args.parcours, donnees)
    print(_couleur("[OK] Case '%s' supprimee, %d reference(s) recablee(s) vers '%s'" % (args.case_id, len(refs), cible), "vert"))
    valider_auto(args, donnees)
    return 0
