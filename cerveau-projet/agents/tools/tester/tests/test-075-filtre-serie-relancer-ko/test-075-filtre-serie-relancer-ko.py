...
tester-lancer-non-regression (v0.5.3) doit revalider UNIQUEMENT les tests
...
Contexte :
  ...
  - v0.5.3 : filtre serie - dans le bloc if args.relancer_ko, si
...
Invariants verifies (sur un registre TEMPORAIRE, jamais le vrai) :
  1. tester-lancer-non-regression --version = v0.5.3
...
def main():
    print("=== Garde-fou : filtre serie de --relancer-ko (tester-lancer-non-regression v0.5.3) ===")

    # 1. Version v0.5.3
    t0 = time.monotonic()
    code, out = run([sys.executable, LANCEUR_PY, "--version"])
    verifier("1. tester-lancer-non-regression --version v0.5.3",
             code == 0 and "v0.5.3" in out, out.strip()[-40:])
