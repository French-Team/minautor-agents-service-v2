...
GARDE-FOU : le RATING (demande utilisateur 2026-08-15) est deploye partout :
  ...
  - le lanceur tester-lancer-non-regression v0.5.3 contient
...
Invariants verifies :
  ...
  6. Le lanceur v0.5.3 contient afficher_rating_fin_de_run (--version v0.5.3)
...
def main():
    ...
    try:
        ...
        # 6. Lanceur v0.5.3 + afficher_rating_fin_de_run
        if point_actif(6):
            t0 = time.monotonic()
            r = lancer([PYTHON, LANCEUR_PY, "--version"])
            contenu = lire(LANCEUR_PY)
            ok = ("v0.5.3" in (r.stdout or "")
                  and "afficher_rating_fin_de_run" in contenu)
            verifier("6. lanceur v0.5.3 + afficher_rating_fin_de_run", ok,
