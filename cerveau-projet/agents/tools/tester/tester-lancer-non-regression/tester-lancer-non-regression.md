...
| `--ko <nouveau\|reprendre>` | SERIE KO PRIORITAIRE (v0.5.5, revue 0.6.0) : fichier persistant `ko-tests.json` qui garde les tests en KO entre les lancements. `reprendre` (defaut) lance D ABORD la serie KO avec SA barriere - les tests qui passent sortent du fichier et ne sont PAS relances dans leur serie d origine ; `nouveau` = MODE BALAYAGE COMPLET : vide le fichier puis lance TOUTES les series SANS arret pour collecter la TOTALITE des KO. Ordre : KO -> A -> B -> C -> D -> E. |
| `--etat-ko` | Affiche le contenu de la serie KO persistante (`ko-tests.json`) puis sort sans lancer |
| `--ko-puis-stop` | CYCLE RAPIDE KO (v0.6.0, demande utilisateur 2026-08-17) : avec `--ko reprendre`, la suite valide UNIQUEMENT la serie KO persistante puis s ARRETE des que sa barriere est franchie (100% verte) - sans relancer les series A-E. Le rapport indique `SERIE KO VERTE = CONTROLE TERMINE` (la suite complete finale n est relancee que si un code partage a ete touche - decision Janus). Si la barriere KO est bloquee, comportement existant (STOP + retour 1). Si `ko-tests.json` est vide, l option est ignoree (avertissement) et la suite se lance normalement. Chrono en mode `barriere-ko` (ne touche jamais la reference globale). |
...
|---|---|---|
| 0.6.2 | 2026-08-17 | ROTATION REGISTRE-TESTS (demande performance, goulot de la suite) : registre-tests.jsonl plafonne a 500 entrees valides (les plus recentes conservees). Avant : 12k+ lignes / 1,9 Mo et re-tri integral a CHAQUE journalisation (~8s par lancement - cause reelle des 60s de test-032). Apres : un lancement filtre passe de ~8s a ~1s. |
| 0.6.1 | 2026-08-17 | CONFIGURATION ADAPTATIVE (demande utilisateur, axe performance) : le lanceur lit `config-environnement.json` (genere par `configurer-environnement`) pour auto-regler les workers et le timeout par test selon les ressources reelles (CPU/RAM/disque/charge). Les options CLI `--workers`/`--timeout-test` restent prioritaires. Remplace le `min(cpu_count, 16)` code en dur. |
| 0.6.0 | 2026-08-17 | CYCLE BALAYAGE + KO TERMINAL (demande utilisateur) : `--ko nouveau` devient MODE BALAYAGE COMPLET (toutes les series sans arret, collecte de la TOTALITE des KO) ; `--ko-puis-stop` affiche `SERIE KO VERTE = CONTROLE TERMINE` au lieu de `VALIDATION FINALE REQUISE` (suite complete finale seulement si code partage touche - decision Janus). |
| 0.5.9 | 2026-08-17 | CYCLE RAPIDE KO (demande utilisateur) : option `--ko-puis-stop` - avec `--ko reprendre`, la suite valide UNIQUEMENT la serie KO persistante puis s ARRETE des que la barriere KO est franchie (100% verte), sans relancer les series A-E (le cycle de correction passe de ~90s a quelques secondes). Rapport `VALIDATION FINALE REQUISE` + retour 0 si 0 KO. Fichier KO vide -> option ignoree (avertissement) + suite normale. Chrono mode `barriere-ko`, reference globale jamais touchee (run partiel). |
...
| 0.5.8 | 2026-08-16 | GARDE-FOU RECHERCHES-WEB : serie e + test-088 (recherches-web conformes + fraiches, anti-recurrence memoire factuelle) + profil outils. |
| 0.5.7 | 2026-08-16 | LISTE BLANCHE DEVELOPPEUR + LECTURE TAGS ROBUSTE : le lanceur lit aussi le format '# Tags:' (commentaire, test-027) comme le garde-fou test-087 - alignement des deux parseurs ; documentation de la liste blanche developpeur du verrou v0.2.2 (vulcain valide ses modifications du lanceur sans attendre janus). |
| 0.5.6 | 2026-08-16 | CATEGORIES PAR TAGS (demande utilisateur) : bloc 'Tags:' dans la docstring de chaque test + categories-tests.json (nom -> tags). Options --tags (OR), --categorie, --desactiver-categorie/--activer-categorie (persistant, config-tests.json), --etat-categories. Outil recommander-series v0.1.0 (croise tags + durees pour reorganiser les series). |
