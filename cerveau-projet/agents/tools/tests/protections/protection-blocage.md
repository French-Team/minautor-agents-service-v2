...
**Chemin :** `agents/tools/tests/protections/`

## Description

Protection qui encadre l'execution des tests pour detecter et stopper les tests qui bloquent : processus qui ne tourne plus (CPU quasi nul), absence de sortie pendant plusieurs intervalles, sortie anormalement longue. Elle complète la protection contre les boucles infinies en detectant les blocages silencieux.

## Utilisation

```bash
# Executer une commande avec protection contre le blocage
source protection-blocage.sh
...
protection-blocage.sh "./mon-outil.sh --test" "Mon test" 60
```
...
| Outil | Complement |
|---|---|
| `protection-boucles-infinies` | Detecte les boucles infinies (CPU eleve) |
| `protection-erreurs-silencieuses` | Detecte les erreurs sans message |
