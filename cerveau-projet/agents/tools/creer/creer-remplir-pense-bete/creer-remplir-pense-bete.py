...
Usage:
  creer-remplir-pense-bete.py [OPTIONS]
"""
...
MARQUEURS = {
    "titre": r"^# Gabarit -- Pense-bete",
    "idee": r"^## 1\. Idee",
    "probleme": r"^## 2\. Probleme / Question",
    "contexte": r"^## 3\. Contexte",
    "liens": r"^## 4\. Liens",
}
...
def remplir_section(fichier, section, contenu, dry_run):
    ...
    if section in TITRE_SECTIONS:
        ...
        lignes[ligne_section] = "# Pense-bete -- " + contenu
    else:
        ...
        if ligne_fin is not None:
            # Section au milieu : garder la ligne de section + la suite a partir de la section suivante
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="creer-remplir-pense-bete",
        description="Remplir une section d'un pense-bete sans ouvrir le fichier.",
    )
    parser.add_argument("fichier", help="Chemin du pense-bete a remplir")
    parser.add_argument("section", help="Section a remplir : titre, idee, probleme, contexte, liens")
    ...
    parser.add_argument("--version", action="version", version="creer-remplir-pense-bete " + VERSION + " (" + STATUT + ")")
