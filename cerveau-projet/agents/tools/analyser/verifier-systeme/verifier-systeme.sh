#!/bin/bash
# verifier-systeme.sh
# Vérifie le système de l'utilisateur et retourne les informations
# Version: 0.1.0-beta
# Date: 2026-08-05
# Auteur: Vulcain
...
aide() {
    ...
    echo "  Vérifie le système de l'utilisateur"
    echo "=========================================="
    echo ""
    ...
    echo "  --format FORMAT     Format de sortie: table, json, resume (défaut: table)"
    echo "  --detail DETAIL     Niveau de détail: standard, complet (défaut: standard)"
...
afficher_table() {
    echo "| Catégorie | Élément | Disponible | Version | Chemin |"
    echo "|---|---|---|---|---|"
    
    # Système
    local sys_info=$(informations_systeme)
    IFS='|' read -r os version arch <<< "$sys_info"
    echo "| Système | OS | $os | $version | - |"
    echo "| Système | Architecture | $arch | - | - |"
...
# Fonction pour afficher en format résumé
afficher_resume() {
    local sys_info=$(informations_systeme)
    IFS='|' read -r os version arch <<< "$sys_info"
    ...
    echo "**Système** : $os $version ($arch)"
    echo "**Shells** : Bash $bash_version"
    echo "**Langages** : Python $python_version, Node.js $node_version"
