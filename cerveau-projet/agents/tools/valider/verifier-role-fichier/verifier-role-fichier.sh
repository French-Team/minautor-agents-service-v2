...
# Vérifie qu'un fichier est utilisé uniquement pour sa fonction prévue
...
if [ -z "$FICHIER" ]; then
    ...
    echo ""
    echo "Vérifie qu'un fichier est utilisé pour sa fonction prévue."
    echo ""
    echo "Exemples:"
...
if [ ! -f "$FICHIER" ]; then
    echo "[ERREUR] Fichier non trouvé : $FICHIER"
    exit 1
fi
...
# Déterminer le rôle du fichier selon son nom
NOM_FICHIER=$(basename "$FICHIER")
ERREURS=0

case "$NOM_FICHIER" in
    index-*.md)
        CONTENU_INTERDIT="^## .*Prochaines étapes|^## .*TODO|^## .*À faire|^## .*Faire|^## .*Statut du|^## .*Corrections récentes|^## .*Notes de session|^## .*Leçons apprises"
        ...
        ;;
    convention-*.md)
        CONTENU_INTERDIT="^## .*Prochaines étapes|^## .*TODO|^## .*À faire|^## .*Faire|^## .*Historique"
        if grep -qE "$CONTENU_INTERDIT" "$FICHIER" 2>/dev/null; then
            LIGNE=$(grep -nE "$CONTENU_INTERDIT" "$FICHIER" | head -1 | cut -d: -f1)
            CONTENU_SUIVANT=$(trouver_ligne_suivante "$FICHIER" "$LIGNE")
            if echo "$CONTENU_SUIVANT" | grep -qE "^\||^\\[|^état|^Type"; then
                : # OK - c'est une définition
            else
        ...
        ;;
    protocole-*.md)
        CONTENU_INTERDIT="^## .*Prochaines étapes|^## .*TODO|^## .*À faire|^## .*Faire|^## .*Statut"
    ...
    spec-*.md)
        ...
        CONTENU_INTERDIT="^## .*Prochaines étapes|^## .*TODO|^## .*À faire|^## .*Faire"
        if grep -qE "$CONTENU_INTERDIT" "$FICHIER" 2>/dev/null; then
...
if [ "$ERREURS" -eq 0 ]; then
    echo "[OK] $FICHIER est conforme à son rôle"
    exit 0
else
    exit 1
fi
