...
# Afficher l'aide
afficher_aide() {
    echo "=== valider-pense-bete v${VERSION} ==="
    echo ""
    echo "Usage: $0 <fichier>"
    echo ""
    echo "Arguments :"
    echo "  <fichier>   Chemin du pense-bete a valider"
    echo ""
    echo "Options :"
    ...
    echo "  2. Le header est present (Statut, ID, Class, Cree, Theme)"
    echo "  3. Les 6 sections sont presentes (1. Idee ... 6. RVAV)"
    echo "  4. Le nommage du fichier est conforme (pense-bete-[theme].[id].[class].[statut].md)"
    echo "  5. Aucun placeholder non remplace ([...] restants)"
    echo "  6. Conformite ASCII (pas d'accents, pas d'emojis)"
    echo ""
    echo "Exemple :"
    echo "  $0 pense-bete-pipeline.001.01.ebauche.md"
    echo ""
}
...
verifier_header() {
    ...
    for champ in "Statut" "ID" "Class" "Cree" "Theme"; do
...
verifier_sections() {
    ...
    for section in "## 1. Idee" "## 2. Probleme / Question" "## 3. Contexte" "## 4. Liens" "## 5. Structure prevue" "## 6. RVAV du pense-bete"; do
...
verifier_nommage() {
    ...
    if echo "$nom" | grep -qE '^pense-bete-[a-z0-9-]+\.[0-9]+\.[0-9]+\.[a-z]+\.md$'; then
        if [ "$verbose" = "true" ]; then
        ...
        echo "  Attendu : pense-bete-[theme].[id].[class].[statut].md"
        return 1
    fi
}
...
main() {
    ...
    if [ "$total_erreurs" -eq 0 ]; then
        echo -e "${GREEN}[OK] Le pense-bete est valide${NC}"
        exit 0
    else
