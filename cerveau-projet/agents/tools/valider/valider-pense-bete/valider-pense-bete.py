...
#   commun: true
"""
valider-pense-bete.py

Verifie l'integrite d'un pense-bete : structure, header, sections,
nommage, placeholders, conformite ASCII.

Utilisation:
  valider-pense-bete.py <fichier> [--verbose] [--help]
...
CHAMPS_HEADER = ["Statut", "ID", "Class", "Cree", "Theme"]
...
PATTERN_NOMMAGE = re.compile(r"^pense-bete-[a-z0-9-]+\.[0-9]+\.[0-9]+\.[a-z]+\.md$")
PATTERN_PLACEHOLDER = re.compile(r"\[[A-Za-z][A-Za-z ]+\]")


def afficher_aide():
    print("=== valider-pense-bete v%s ===" % VERSION)
    print("")
    print("Usage: valider-pense-bete.py <fichier>")
    print("")
    print("Arguments :")
    print("  <fichier>   Chemin du pense-bete a valider")
    print("")
    print("Options :")
    ...
    print("  2. Le header est present (Statut, ID, Class, Cree, Theme)")
    print("  3. Les 6 sections sont presentes (1. Idee ... 6. RVAV)")
    print("  4. Le nommage du fichier est conforme (pense-bete-[theme].[id].[class].[statut].md)")
...
def verifier_nommage(fichier, verbose):
    ...
    print("  Attendu : pense-bete-[theme].[id].[class].[statut].md")
    return 1
...
def main(argv):
    ...
    while i < len(argv):
        ...
        if arg == "--verbose":
            ...
            print("valider-pense-bete v%s (%s)" % (VERSION, STATUT))
            return 0
        elif arg.startswith("-"):
    ...
    print("=== Validation du pense-bete ===")
    ...
    print("=== Resume ===")
    if total_erreurs == 0:
        print("[OK] Le pense-bete est valide")
        return 0
