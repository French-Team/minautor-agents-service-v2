#!/bin/bash
# rechercher-specs.sh
# Outil pour rechercher les specs existantes et eviter les doublons
# Version : 0.1.0-beta
# Statut : ebauche
...
# Afficher l'aide
afficher_aide() {
    echo "=== rechercher-specs v${VERSION} ==="
    echo ""
    echo "Usage: $0 [OPTIONS]"
    echo ""
    echo "Options :"
    echo "  --theme <motif>   Rechercher les specs dont le theme est identique ou proche"
    echo "  --tous            Lister toutes les specs existantes (inventaire complet)"
...
main() {
    ...
    # Recueillir toutes les specs (fichiers <prefixe>-*.md, hors templates et index)
    local fichiers=()
    ...
    if [ ${#fichiers[@]} -eq 0 ]; then
        echo -e "${YELLOW}[ATTENTION] Aucune ${LABEL} trouvee${NC}"
        exit 0
    fi
