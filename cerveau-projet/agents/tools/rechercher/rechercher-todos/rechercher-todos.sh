#!/bin/bash
# rechercher-todos.sh
# Outil pour rechercher les todos existants et eviter les doublons
# Version : 0.2.0
...
# Afficher l'aide
afficher_aide() {
    echo "=== rechercher-todos v${VERSION} ==="
    echo ""
    echo "Usage: $0 [OPTIONS]"
    echo ""
    echo "Options :"
    echo "  --theme <motif>   Rechercher les todos dont le theme est identique ou proche"
    echo "  --tous            Lister tous les todos existants (inventaire complet)"
...
main() {
    ...
    # Recueillir tous les todos (fichiers <prefixe>-*.md, hors templates et index)
    local fichiers=()
