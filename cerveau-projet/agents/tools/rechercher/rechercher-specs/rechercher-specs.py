...
Rechercher les specs existantes et eviter les doublons.

Options:
  --theme <motif>   Rechercher les specs dont le theme est identique ou proche
  --tous            Lister toutes les specs existantes (inventaire complet)
...
def afficher_aide():
    print("=== rechercher-specs v%s ===" % VERSION)
    print("")
    print("Usage: rechercher-specs.py [OPTIONS]")
    print("")
    print("Options :")
    print("  --theme <motif>   Rechercher les specs dont le theme est identique ou proche")
    print("  --tous            Lister toutes les specs existantes (inventaire complet)")
...
def main(argv):
    ...
    while i < len(argv):
        ...
        if arg in ("--help", "--aide", "-h"):
            ...
            continue
        if arg == "--version":
            print("rechercher-specs v%s (%s)" % (VERSION, STATUT))
            return 0
    ...
    if not fichiers:
        print("[ATTENTION] Aucune %s trouvee" % LABEL)
        return 0
