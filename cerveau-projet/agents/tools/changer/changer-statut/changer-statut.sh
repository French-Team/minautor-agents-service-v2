...
afficher_aide() {
    ...
    echo "  --force       Forcer le changement meme si des liens pointent vers le fichier"
...
# Fonction pour verifier si le statut est valide
statut_valide() {
    local statut=$1
    case "$statut" in
...
extraire_parties() {
    ...
    # Le separateur est le point
    local parties=$(echo "$basename" | awk -F'.' '{
        # Nombre total de parties
        n = NF;
        
        # Statut = derniere partie
        statut = $n;
        
        # Class = avant-derniere partie
        class = $(n-1);
        
        # Id = troisieme partie depuis la fin
        id = $(n-2);
        
        # Nom = tout sauf les 3 dernieres parties
        nom = "";
        for (i = 1; i <= n-3; i++) {
            if (i > 1) nom = nom ".";
            nom = nom $i;
...
trouver_liens() {
    ...
    # Chercher les liens dans le meme dossier et les sous-dossiers
...
# Fonction pour incrementer le class
incrementer_class() {
    local class=$1
...
changer_statut() {
    ...
    # Verifier qu'on a bien extrait les informations
    ...
    # Verifier si des liens pointent vers ce fichier
    echo -e "${BLUE}--- Verification des liens ---${NC}"
    ...
    if [ -n "$liens" ]; then
        ...
        echo -e "${GREEN}[OK] Aucun lien trouve${NC}"
    fi
    
    # Verifier que le nouveau nom n'existe pas deja
    if [ -f "$nouveau_chemin" ]; then
        echo -e "${RED}[ERREUR] Le fichier existe deja : $nouveau_chemin${NC}"
        exit 1
    fi
    ...
    if [ "$DRY_RUN" = true ]; then
        echo -e "${YELLOW}[DRY-RUN] Changement non applique${NC}"
        echo "  Renommer : $(basename "$fichier") -> $nouveau_nom"
    else
        # Renommer le fichier
        mv "$fichier" "$nouveau_chemin"
        if [ $? -eq 0 ]; then
            echo -e "${GREEN}[OK] Fichier renomme avec succes${NC}"
