...
aide() {
    ...
    echo "  --type TYPE         Type de fichier: bash, python, js (defaut: auto)"
    echo "  --version           Afficher la version"
    echo ""
    echo "Arguments:"
    echo "  FICHIER             Fichier a analyser"
    echo ""
    echo "Exemples:"
...
# Fonction pour detecter le type de fichier
detecter_type() {
    local fichier=$1
...
# Detection du type si necessaire
if [[ "$TYPE" == "auto" ]]; then
