...
compter_outils_categorie() {
    ...
    # Cas special tests : template-test.md + protections/*.md
    if [ "$categorie" = "tests" ]; then
        nb=0
        [ -f "$dir/template-test.md" ] && nb=$((nb + 1))
...
lister_outils_categorie() {
    ...
    # Cas special tests
    if [ "$categorie" = "tests" ]; then
        if [ -f "$dir/template-test.md" ]; then
            liste="template-test + protections : "
            local first=""
            for f in "$dir/protections"/*.md; do
                [ -f "$f" ] || continue
                local nom=$(basename "$f" .md | sed 's/^protection-//')
                if [ -z "$first" ]; then
                    liste="${liste}${nom}"
                    first="1"
                else
...
compter_total_outils() {
    local total=0
    for cat in explorer valider analyser corriger creer generateurs tests templates; do
...
verifier() {
    ...
    for cat in Explorer Valider Analyser Corriger Creer Generateurs Tests; do
        local cle=$(echo "$cat" | tr '[:upper:]' '[:lower:]')
...
mettre_a_jour() {
    ...
    for agent in $(lister_agents_reels); do
        if ! grep -qi "\*\*${agent}\*\*" "$README"; then
            ...
            # Insérer la ligne avant '### Le cycle fondamental' (fin de la table des agents)
