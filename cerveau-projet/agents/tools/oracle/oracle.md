...
|---|---|---|
| 0.5.13 | 2026-09-05 | PROTOCOLE LANGUE UTILISATEUR (decision utilisateur 2026-09-05) : les missions relayees (`_envoyer_direct`) sont prefixees du rappel de langue lu dans USER-PROFIL.md racine (`[LANGUE] ...`) pour que l agent voie la langue des la reception, avant meme le pilote. `pilote.py` v0.2.5 : `_rappel_langue()` + `_prefixe_mission()` + `_sans_prefixe_langue()`. `files.py` `_consommer_notification_mission` : comparaison tolerant au prefixe (symetrique, la consommation a la TERMINEE ne casse pas - fix 0.5.11 preserve). |
