...
identite:
  ...
  commun: true
---
# Spec -- Guide-Parcours (jeu de piste) v0.6.3
...
| Texte court (<= 100 car.) pour un rappel specifique a la case | Texte inline long qui duplique une regle existante |
...
3. `valider-case` signale en A ALLEGER : (a) tout texte inline > 160 car.
   (plafond absolu d'un indice), ou (b) une case dont le BUDGET PONDERE des
   indices depasse 3,0 unites -- indice COURT (<= 100 car. ou sans texte)
   = 0,5, indice LONG (> 100 car.) = 1, donc 2 courts valent 1 long
   (6 courts = 3,0 OK ; 3 longs = 3,0 OK ; 4 longs = 4,0 signale).
   Tout indice `ref` non resolvable est aussi signale (verifier_references).
...
10. **AUCUNE BOUCLE D'ATTENTE (v0.2.10)** : une branche qui revient sur la MEME
    ...
    d'attente.
11. **NOMMAGE DES IDS DE CASES (v0.6.2)** : l'id de chaque case suit la
    convention ETENDUE `c[<prefixe-alpha-maj>]<numero>[a-z]?` :
    - cas normal : `c` + numero + suffixe minuscule optionnel (`c0`, `c12b`,
      `c29d`) ;
    - prefixe thematique MAJUSCULE optionnel d'UNE lettre : `cT1`..`cT10`
      (T = ligne Trio de Janus, decision utilisateur 2026-08-11 : conserver
      les ids cT*) ;
    - le suffixe reste en minuscules ; AUCUNE ponctuation (jamais de point) ;
    - source de verite : valider-case v1.1.0 (regex `^c[A-Z]?\d+[a-z]*$`),
      qui REJETTE tout id non conforme (message NOMMAGE).
...
## Patterns valides en production (v0.2.0, v0.2.4, v0.2.5, v0.2.6, v0.2.8, v0.2.13, v0.2.15, v0.2.16, v0.2.18, v0.2.19, v0.2.22, v0.2.23, v0.2.24, v0.2.27, v0.2.29)
...
v0.2.19, Pattern 12 depuis v0.2.22, Pattern 13 depuis v0.2.23, Pattern 14
depuis v0.2.24, Pattern 15 depuis v0.2.27).
...
5. ERREUR HORS-PERIMETRE -> INTER-ROUND (v0.6.3, decision utilisateur
   2026-08-22) : un agent qui detecte une erreur hors-perimetre n'interrompt
   PAS le round et ne reactive PAS Cerberus : il active L'AGENT HABILITE avec
   le rapport de l'erreur. A la fin de l'inter-round, l'habilite REACTIVE
   l'agent qui avait lance l'inter-round, qui REPREND son round principal.
   Cascade autorisee si l'erreur concerne l'habilite lui-meme ou si la
   reparation en revele une autre : les habilites s'enchainent DANS
   l'inter-round, le dernier reactive l'appelant. Themis/Janus restent hors
   des inter-rounds courts (Janus garde la non-regression finale). Une erreur
   n'est JAMAIS 'seulement detectee' : reparation exclusive par l'habilite.
   Reference : protocole-fin-mission v0.2.0 section ROUND/INTER-ROUND/REPRISE.
...
### Pattern 15 -- JAMAIS D'ARRET APRES L'ACTIVATION : l'activation ne clot PAS le tour (v0.2.27)

Le cerveau-projet peut tourner en deux modes, mais la regle est la MEME dans
les deux. En MODE MONO-LLM (une seule conversation, un seul LLM qui joue tous
les roles), l'activation ne transfere PAS l'execution : l'outil
activer-agent-principal.py ne fait AUCUN sous-processus (0 subprocess/
os.system/Popen/exec) -- il ecrit 3 fichiers de trace (AGENTS.md,
AGENTS-historique.md, classeur-variables) et documente le changement de role.
En MODE MULTI-LLM (plusieurs sessions, plusieurs LLM), chaque LLM travaille EN
PARALLELE dans SA propre session (son id via sidentifier, sa session
session-llm-N) : l'activation d'un agent dans une session documente le role
courant de CETTE session uniquement -- elle ne DELEGUE JAMAIS l'execution a un
autre LLM. Il n'existe AUCUN mecanisme de relais : un LLM qui s'arrete apres
l'activation bloque sa mission, personne ne reprend a sa place (les autres LLM
travaillent en parallele sur LEURS missions, pas sur la sienne).
Lecon utilisateur 2026-08-09 : 2 missions consecutives se sont arretees apres
l'activation de Themis (l'utilisateur devait dire continue) -- la carte de
Cerberus case c10 ordonne pourtant de continuer (suivant c7), l'outil
fonctionnait, mais le LLM a clos son tour en imitant un pretendu protocole de
relais qui n'existe pas.

**Regles** :
1. APRES l'activation d'un agent, l'agent active est JOUE IMMEDIATEMENT dans
   le MEME tour : relecture de SA fiche + SES corrections (c0/c0b), puis
   mission, puis reactiver/activer le suivant (Pattern 13). Pas de message
   d'arret type 'Themis active, il prend le relais' qui bloque la mission.
2. L'activation documente le role (3 fichiers de trace), elle ne transfere
   pas l'execution : le LLM courant continue a jouer le role active.
3. L'arret apres activation est TOUJOURS fautif -- dans AUCUN mode (ni mono,
   ni multi-LLM). S'arreter = bloquer la mission jusqu'a un continue de
   l'utilisateur, car personne d'autre ne reprend (les autres LLM travaillent
   en parallele sur leurs propres sessions/missions).
4. Le Pattern 13 (la fin suit SA carte) s'applique aussi a l'activation :
   l'activation est un MAILLON (on continue avec l'agent active), pas une fin.
5. En mode multi-LLM, la regle est identique : l'activation de SON agent (dans
   SA session) se joue immediatement dans le meme tour. Les autres LLM, en
   parallele, font de meme avec leurs propres missions -- aucun ne vient
   reprendre la mission d'un autre.

**Lien avec les autres patterns** : complete le Pattern 8 (chaine de
delegation bout-en-bout) et le Pattern 13 (la fin suit SA carte) en precisant
le comportement EXACT attendu entre l'activation et la mission : meme tour,
pas d'arret, dans AUCUN mode (mono comme multi-LLM).
...
### Pattern 16 -- ALLEGEMENT DES CASES SURCHARGEES (references resolues + combos, anti-doublon) (v0.2.29)

Le Probleme : une case peut devenir surchargee (valider-case : budget pondere
des indices depasse 3,0 unites -- court <= 100 car. = 0,5 / long > 100 = 1 --
ou texte de regle > 160 caracteres) quand on accumule des regles, des
protocoles et des outils dans une meme case. La surcharge noie
...
marquee A ALLEGER est candidate (seuils : poids des indices > 3,0 unites,
ou texte de regle > 160 caracteres).
...
- Sequence d'outils / d'etapes (poids des indices > 3,0 unites) -> LEVIER B : combo
...
## Procedure d'audit des 16 patterns (v0.2.1, v0.2.4, v0.2.5, v0.2.6, v0.2.8, v0.2.13, v0.2.15, v0.2.16, v0.2.18, v0.2.19, v0.2.22, v0.2.23, v0.2.24, v0.2.27, v0.2.29)
...
### 4c. RE-AUDIT COMPLET DES 15 PATTERNS (v0.2.27, LECON THEMIS)
...
1. Apres avoir audite le pattern nouveau ou modifie (4b par exemple),
   ...
   carte), 4l (verification d impact) et 4m (mode mono-llm) sur le MEME
   parcours.
2. Ne PAS conclure au verdict global tant que les 15 procedures n ont pas ete
...
4. Appliquer TOUS les criteres d'acceptation 1 a 26 (criteres 13 fin passive,
   22 conformite d execution, 24 la fin suit SA carte, 25 verification
   d impact, 26 mode mono-llm) en complement : la conformite d'un parcours =
   TOUS les criteres, pas seulement ceux lies au pattern recent.
5. La procedure 4l (verification d impact) est un pas OBLIGATOIRE du re-audit
   ...
   modifies (Pattern 14).
6. La procedure 4m (mode mono-llm) verifie que l'execution reelle de la
   mission a continue dans le MEME tour apres l'activation de l'agent
   (Pattern 15, v0.2.27) : pas de message d'arret type 'X est active, il
   prend le relais' qui bloque la mission (lecon utilisateur 2026-08-09).
...
14. RE-AUDIT COMPLET DES 15 PATTERNS : a chaque creation/modification/audit,
    les procedures 1, 2, 3, 4, 4b, 4c, 4d, 4e, 4f, 4g, 4h, 4i, 4j, 4k, 4l et
    4m sont REJOUES integralement (jamais la procedure nouvelle seule) ; le
    verdict global n'est prononce qu'apres le re-audit complet (v0.2.7,
    generalise a 15 patterns en v0.2.27 -- lecon Themis : 3 ecarts ASCII
...
26. JAMAIS D'ARRET APRES L'ACTIVATION (v0.2.27) : apres l'activation d'un
    agent, l'agent active a-t-il ete JOUE IMMEDIATEMENT dans le MEME tour
    (relecture fiche + corrections, puis mission, puis reactiver/activer le
    suivant) au lieu d'un arret qui bloque la mission ? L'arret apres
    activation est TOUJOURS fautif, dans AUCUN mode (mono comme multi-LLM :
    les LLM travaillent en parallele chacun dans sa session, aucun relais
    n'existe -- Pattern 15, lecon utilisateur 2026-08-09 : 2 missions
    arretees apres l'activation de Themis, la carte c10 ordonnait pourtant de
    continuer).
