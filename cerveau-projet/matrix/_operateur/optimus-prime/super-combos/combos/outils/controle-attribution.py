...
LE CONTROLE, EN DEUX PIECES :
  ...
  (b) UN CONTROLE PERMANENT (`verifier`) -- pour chaque source dont l'empreinte a
      change depuis la pose, il faut une NOTE TRACEE posterieure a la pose. Sans
      note : ECRITURE HORS DE SA PORTE, accusee nommement (fichier + empreinte
      attendue + empreinte mesuree). Code 1.
...
routines -- et les zones jetables ou d'archive : elles sont ecrites par LEURS
PROPRES portes. Chaque exclusion a son motif et son COMPTE, imprimes a chaque
passe : une exclusion muette serait un angle mort (L-104).
...
def domiciles():
    ...
    for chemin_module in DOMICILES_VUES:
        ...
        if not nom:
            continue
        # La vue du suivi d'Optimus vit dans le dossier de l'OPERATEUR
        # (`_operateur/optimus-prime/`), a cote de son outil.
...
def notes_du_domicile(maison):
    """{cle canonique: [dates de note]} lu au DOMICILE des modifications.
    ...
    for cle, entree in (donnees.get("fichiers") or {}).items():
        ...
        if not canonique:
            continue
        dates = [str(item.get("date", "")) for item in (entree or {}).get("modifications", [])]
        notes.setdefault(canonique, []).extend(dates)
    return notes


def ecritures_non_attribuees(reference, actuelles, notes, date_pose):
    """[(cle, empreinte attendue, empreinte mesuree)] : change SANS note posterieure.
    ...
    TROIS populations, et une seule est accusee :
      ...
      - changee : elle doit porter une note STRICTEMENT posterieure a la pose.
    Une note ANTERIEURE a la pose ne blanchit rien : c'est l'ecriture d'un autre
    tour, et c'est precisement l'erreur que le controle existe pour attraper.
    """
    accusees = []
    for cle in sorted(reference):
        ...
        if mesuree is None or mesuree == attendue:
            continue
        dates = notes.get(cle) or []
        if any(date > date_pose for date in dates):
            continue
        accusees.append((cle, attendue, mesuree))
    return accusees


def creations_non_attribuees(reference, actuelles, notes, date_pose):
    """[cle] des sources NEUVES (absentes de la pose) sans note posterieure.
...
def verifier():
    ...
    for cle, attendue, mesuree in accusees:
        print("  - " + cle)
        print("      empreinte attendue : " + attendue)
        print("      empreinte mesuree  : " + mesuree)
        print("      aucune note posterieure a la pose au domicile bdd-modifications")
    for cle in neuves:
...
def auto_test():
    ...
    # 1. COBAYE POSITIF : l'ecriture PASSE PAR LA PORTE (note posterieure a la pose).
    apres = {cle: [pose, "2026-09-22 10:05:00"]}
    accusees = ecritures_non_attribuees(reference, changee, apres, pose)
    controler("le cobaye EPARGNE une ecriture notee APRES la pose", not accusees,
              str(len(accusees)) + " accusation(s)")
    ...
    anterieure = {cle: ["2026-09-01 08:00:00", "2026-09-02 09:00:00"]}
    ...
    # 4. LE CONTRE-TEMOIN : sans le controle, la MEME ecriture passe EN SILENCE.
    #    Le temoin brut -- l'empreinte seule, ce que le projet savait faire -- voit
    #    le changement et ne sait RIEN de la porte : l'ecriture legitime et
    ...
    # 5. LE TEMOIN QUI NE DOIT PAS MENTIR : inchangee = hors du controle, et une
    ...
    # 7. LA CREATION HORS PORTE : le fichier n'existe pas a la pose, donc aucun
    #    changement d'empreinte ne peut le trahir -- c'est la population qu'un
    #    controle par empreintes laisserait passer ENTIEREMENT.
    neuve = "_operateur/optimus-prime/outil-fantome.py"
    avec = dict(changee)
    avec[neuve] = "C" * 64
    controler("le cobaye MORD une source NEUVE sans note",
              creations_non_attribuees(reference, avec, {}, pose) == [neuve], neuve)
    controler("le contre-temoin EPARGNE une source neuve NOTEE apres la pose",
              not creations_non_attribuees(reference, avec, {neuve: ["2026-09-22 10:01:00"]}, pose))
    controler("une source deja posee n'est jamais comptee comme neuve",
              not creations_non_attribuees(reference, reference, {}, pose))

    # 6. LES EXCLUSIONS SONT DECLAREES, ET ELLES MORDENT LA OU ELLES DOIVENT.
