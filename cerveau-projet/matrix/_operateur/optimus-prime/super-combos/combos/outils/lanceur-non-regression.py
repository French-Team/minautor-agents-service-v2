...
def main():
    ...
    for commande, nom, remede in pre_vol:
        if not Path(commande[0]).is_file():
            ...
            continue
        resultat = subprocess.run([sys.executable, str(commande[0]), *commande[1:]],
    ...
    for t in themes:
        r = subprocess.run([sys.executable, str(outils / "tester-theme.py"), str(t)],
                           capture_output=True, text=True)
    ...
    # 3. cartographe
    print("== 3. cartographe ==")
    r = subprocess.run([sys.executable, str(outils / "cartographe-matrice.py"), "--racine", str(zone)],
                       capture_output=True, text=True)
    etat = "OK" if r.returncode == 0 else "KO"
    print(f"  carte: {etat}")
    if r.returncode != 0:
    ...
    print("== 4. garde-ascii ==")
    r = subprocess.run([sys.executable, str(outils / "garde-ascii.py"), str(zone)],
                       capture_output=True, text=True)
    etat = "OK" if r.returncode == 0 else "KO"
    print(f"  ascii: {etat}")
    if r.returncode != 0:
    ...
    if not verificateur.is_file():
        ...
        r = subprocess.run([sys.executable, str(verificateur), "prefixes"],
                           capture_output=True, text=True)
        etat = "OK" if r.returncode == 0 else "KO"
    ...
    if not lanceur_flux.is_file():
        ...
        r = subprocess.run([sys.executable, str(lanceur_flux), "--racine", str(racine_flux)],
                           capture_output=True, text=True)
        etat = "OK" if r.returncode == 0 else "KO"
        print(f"  flux: {etat}")
        if r.returncode != 0:
    ...
    if not garde_attente.is_file():
        ...
        r = subprocess.run([sys.executable, str(garde_attente), "--racine", str(zone.parent.parent)],
                           capture_output=True, text=True)
        etat = "OK" if r.returncode == 0 else "KO"
    ...
    if not suivi_optimus.is_file():
        ...
        r = subprocess.run([sys.executable, str(suivi_optimus), "verifier"],
                           capture_output=True, text=True)
        etat = "OK" if r.returncode == 0 else "KO"
        print(f"  trace: {etat}")
        if r.returncode != 0:
    ...
    if not garde_anti_spam.is_file():
        ...
        r = subprocess.run([sys.executable, str(garde_anti_spam), "--racine", str(zone.parent.parent)],
                           capture_output=True, text=True)
        etat = "OK" if r.returncode == 0 else "KO"
    ...
    if not garde_chapitres.is_file():
        ...
        r = subprocess.run([sys.executable, str(garde_chapitres), "--racine", str(zone.parent.parent)],
                           capture_output=True, text=True)
        etat = "OK" if r.returncode == 0 else "KO"
    ...
    if not garde_exemptions.is_file():
        ...
        r = subprocess.run([sys.executable, str(garde_exemptions), "--racine", str(zone.parent.parent)],
                           capture_output=True, text=True)
        etat = "OK" if r.returncode == 0 else "KO"
    ...
    if not garde_rotation.is_file():
        ...
        r = subprocess.run([sys.executable, str(garde_rotation), "--racine", str(zone.parent.parent)],
                           capture_output=True, text=True)
        etat = "OK" if r.returncode == 0 else "KO"
    ...
    if not garde_historique.is_file():
        ...
        r = subprocess.run([sys.executable, str(garde_historique), "--racine", str(zone.parent.parent)],
                           capture_output=True, text=True)
        etat = "OK" if r.returncode == 0 else "KO"
    ...
    if not garde_observations.is_file():
        ...
        r = subprocess.run([sys.executable, str(garde_observations), "--racine", str(zone.parent.parent)],
                           capture_output=True, text=True)
        etat = "OK" if r.returncode == 0 else "KO"
    ...
    if not garde_passes.is_file():
        ...
        r = subprocess.run([sys.executable, str(garde_passes), "--racine", str(zone.parent.parent)],
                           capture_output=True, text=True)
        etat = "OK" if r.returncode == 0 else "KO"
        print(f"  passes: {etat}")
        if r.returncode != 0:
    ...
    if not sc001.is_file():
        ...
        r = subprocess.run([sys.executable, str(sc001), "auto-test"],
                           capture_output=True, text=True)
        etat = "OK" if r.returncode == 0 else "KO"
        print(f"  sc-001: {etat}")
        if r.returncode != 0:
    ...
    if not garde_cadence.is_file():
        ...
        r = subprocess.run([sys.executable, str(garde_cadence), "--racine", str(zone.parent.parent)],
                           capture_output=True, text=True)
        etat = "OK" if r.returncode == 0 else "KO"
    ...
    if not garde_parcours.is_file():
        ...
        r = subprocess.run([sys.executable, str(garde_parcours), "--racine", str(zone.parent.parent)],
                           capture_output=True, text=True)
        etat = "OK" if r.returncode == 0 else "KO"
    ...
    if not garde_chemins.is_file():
        ...
        r = subprocess.run([sys.executable, str(garde_chemins), "chemins", "--strict"],
                           capture_output=True, text=True)
        etat = "OK" if r.returncode == 0 else "KO"
    ...
    if not garde_cartes.is_file():
        ...
        r = subprocess.run([sys.executable, str(garde_cartes), "--racine", str(zone.parent.parent)],
                           capture_output=True, text=True)
        etat = "OK" if r.returncode == 0 else "KO"
        print(f"  cartes: {etat}")
        if r.returncode != 0:
    ...
    if not garde_roles.is_file():
        ...
        r = subprocess.run([sys.executable, str(garde_roles), "--racine", str(zone.parent.parent)],
                           capture_output=True, text=True)
        etat = "OK" if r.returncode == 0 else "KO"
        print(f"  roles: {etat}")
        if r.returncode != 0:
    ...
    if not garde_resolution.is_file():
        ...
        for verbe in ("resolution", "insertions"):
            r = subprocess.run([sys.executable, str(garde_resolution), verbe,
                                "--racine", str(zone.parent.parent)],
                               capture_output=True, text=True)
            etat = "OK" if r.returncode == 0 else "KO"
            print(f"  {verbe}: {etat}")
