...
3. la regle est LUE a l allumage : elle figure a ses 4 points de relecture
...
TMP_DIRS = {"tmp-optimus", "tmp-cameleon"}
...
def controler_residus(racine: Path) -> list:
    ...
    for p in racine.rglob("*"):
        ...
        if any(part in EXCLUS_DIRS for part in rel.parts):
            continue
        if rel.parts and rel.parts[0] in TMP_DIRS:
            continue
...
def controler_zones(racine: Path) -> tuple:
    """Zones tmp-*/ : ecarts (README absent) + information (nb de fichiers)."""
    ecarts = []
    infos = []
    for nom in sorted(TMP_DIRS):
        zone = racine / nom
        if not zone.is_dir():
            continue
        if not (zone / "README.md").is_file():
            ecarts.append(
                nom + "/ present SANS son README (zone privee de son contrat)"
            )
        fichiers = [
            p for p in zone.rglob("*")
            if p.is_file() and "__pycache__" not in p.parts
        ]
        infos.append(nom + "/ : " + str(len(fichiers)) + " fichier(s) present(s)")
    return ecarts, infos
...
def main():
    ...
    ecarts_zones, infos = controler_zones(racine)
    ...
    print("Tmp sain : aucun residu hors tmp-optimus/tmp-cameleon, README des zones "
          "present, regle perimetre-tmp lue a ses 4 points.")
    return 0
