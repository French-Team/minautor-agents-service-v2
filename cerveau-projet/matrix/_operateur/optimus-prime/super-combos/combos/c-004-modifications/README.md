---
identite:
  type: readme
  appartient_a: optimus-prime
  commun: false
---

# c-004-modifications

> **Combo numerote c-004** -- le numero est OBLIGATOIRE et jamais reutilise
> (contrat de nommage obligatoire CV-008 -- BDD conventions-matrice,
> decision createur 2026-09-13).

Combo dedie a la BDD **Modifications par fichier (modifications-par-fichier.json)** (`modifications`).

> Modifications par fichier + tags : savoir en une lecture ce qui a ete fait sur un fichier (hash avant/apres, raison).
> Genere par `creer-combo.py` (imperatif 46) le 2026-09-13 10:43:41.

## Chaine

1. **lire** : outil `matrice/data/outils/bdd-modifications/main.py`, commande `lire` (filtre `--tag <tag>`)
2. **normaliser** : compte les lignes utiles
3. **noter usage** : piste dans `matrice/data/outils/bdd-usages/main.py` (tags `combo,bdd,modifications`)

## Usage

```bash
python3 cerveau-projet/matrix/_operateur/optimus-prime/super-combos/combos/c-004-modifications/main.py executer [--tag <tag>] [--mission <id>]
python3 cerveau-projet/matrix/_operateur/optimus-prime/super-combos/combos/c-004-modifications/main.py lire [--tag <tag>]
python3 cerveau-projet/matrix/_operateur/optimus-prime/super-combos/combos/c-004-modifications/main.py status
```

## Tags de la BDD

optimus,reparation
