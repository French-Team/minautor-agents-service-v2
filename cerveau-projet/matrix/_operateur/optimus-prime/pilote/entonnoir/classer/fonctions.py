...
def mission_du_vrac(etat, identifiant):
    """Retourne la mission EO-XXX du vrac, ou None (une seule tache : chercher)."""
...
def classer_mission(etat, identifiant, type_cible, categorie_explicite, role_explicite=""):
    """Classe la mission EO-XXX du vrac vers SA file-type (echelons 1-2).
    ...
    ROLE (L-061/MO-076) : le classement pose le ROLE de la mission, champ
    FERME (le vivier). Explicite (`--role`) ou PROPOSE par la table complete
    (roles.py) sur le couple (type, categorie) -- jamais devine, toujours
    imprime pour etre confirme ou corrige. Un role hors vivier est REFUSE
    (code 2) : un item sans role ferait echouer l'injection plus tard.
    ...
    # ROLE : propose par la table complete, ou impose par la declaration.
    source_role = "declaration" if role_explicite else "table"
    role_brut = role_explicite or proposer_role(type_cible, categorie_cible)
    code_role, role_canonique, ecart_role = valider_role(role_brut)
    if ecart_role:
        print(ecart_role)
    if code_role != 0:
        return code_role, (
            "Mission " + identifiant + " NON classee : le role rendu (" + repr(role_brut)
            + ") n'est pas un theme du vivier."
        )
