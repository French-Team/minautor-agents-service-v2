"""Categorie rotation : borne le journal de la veille en ARCHIVANT ses anciens.
...
connu" -- lecon L-040, course refusee) vit a UN seul endroit. La veille ne
declare que SON journal et SES constantes.

Mesure d'origine (2026-09-13) : journal-veille.txt pesait 3,85 Mo / 44 050
lignes et croissait d'environ 0,5 Mo par jour, sans aucune borne -- alors qu'un
troisieme lecteur entier l'a ete mesure : `journal-multi-encarts` en lisait
3,85 Mo pour n'en garder que 5 evenements.
"""
...
def executer(arguments):
    """Verbe `rotation` : borne le journal de la veille en archivant ses anciens."""
    return cli(
        arguments,
        chemin_journal,
        SEUIL_OCTETS_JOURNAL,
