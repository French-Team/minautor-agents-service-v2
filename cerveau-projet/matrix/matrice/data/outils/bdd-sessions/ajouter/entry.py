"""Categorie ajouter : orchestre l'enregistrement d'une sessions taguee.
...
NOMS_OPTIONS = ("session", "tags", "source")


def executer(arguments):
    options = extraire_options(arguments, NOMS_OPTIONS)
    contenu = options.get("session", "")
    tags = separer_tags(options.get("tags", ""))
    source = options.get("source", "")

    if not contenu or not tags:
        print('Usage : python main.py ajouter --session "..." --tags "a,b" [--source "..."]')
        return 2
    ...
    print(
        "Sessions " + entree["id"] + " enregistree (tags : " + ", ".join(entree["tags"])
