"""Entree du verbe auditer : lire les options, valider le plan, rendre le rapport.

Role : ORCHESTRER (options -> plan -> premisses -> audit). Aucune regle de classe ici.
"""
from auditer.fonctions import executer
...
NOMS_CONNUS = ("plan", "json", "perimetre")
DRAPEAUX = ("json",)
...
def executer_verbe(arguments):
    ...
    if ecarts:
        ...
        for ecart in ecarts:
            print("  - " + ecart)
        return 2
    return executer(plan, brut=bool(options.get("json")), perimetre=options.get("perimetre"))
