...
- **verifier** : controler l'integrite de la BDD par empreinte SHA-256.

## Options

```
python main.py noter --fichier <chemin> --action <cree|modifie|corrige|supprime>
                     --detail "..." --tags "tag1,tag2"
python main.py lire  [--fichier <chemin>] [--tag <tag>]
python main.py verifier
```

- `--fichier` : chemin relatif a `matrix/` (ex: `matrice/data/data-readme.md`).
...
| `commun.py` | fonctions communes : charger, enregistrer (atomique), empreinte |
| `noter/`, `lire/`, `verifier/` | categories : `entry.py` orchestre, `fonctions.py` fait |
