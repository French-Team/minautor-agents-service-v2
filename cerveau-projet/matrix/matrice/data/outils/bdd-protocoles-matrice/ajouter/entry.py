"""Categorie ajouter : orchestre l'enregistrement d'une le protocole taguee.
...
NOMS_OPTIONS = ("protocole", "tags", "source")


def executer(arguments):
    options = extraire_options(arguments, NOMS_OPTIONS)
    contenu = options.get("protocole", "")
    tags = separer_tags(options.get("tags", ""))
    source = options.get("source", "")

    if not contenu or not tags:
        print('Usage : python main.py ajouter --protocole "..." --tags "a,b" [--source "..."]')
        return 2
    ...
    print(
        "Le protocole " + entree["id"] + " enregistree (tags : " + ", ".join(entree["tags"])
